...
export function evaluate(staged, read) {
  ...
  for (const path of closes) {
    ...
    const rows = Array.isArray(close.planRows) ? close.planRows : typeof close.planRows === "string" ? close.planRows.split(/[,\s]+/) : [];
    const gated = rows.map((r) => String(r).trim()).filter((r) => GATED_ROW.test(r));
    if (gated.length === 0) continue; // not an OPS-23 close: out of scope by design
    const cite = close.probe?.artifact;
    if (typeof cite !== "string" || !cite.trim()) {
      problems.push(`${path}: OPS-23 rows ${gated.join(",")} closed with no probe.artifact. R-4: a lane closes on a surface-probe artifact, never on a merge, a cortex read, a ledger count or an MCP read.`);
      continue;
    }
    ...
    for (const row of gated) {
      ...
      if (notPass.length) {
        problems.push(`${path}: ${row} is not PASS in ${cite}: ${notPass.map((r) => `${r.parcel} ${r.verdict}`).join("; ")}. UNMEASURED is not a pass.`);
      }
    }
  }
