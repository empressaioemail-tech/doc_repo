...
Plus a generation-persists-citations acceptance test (proven, not assumed). **Status (2026-06-09): the gate-deposit loop is CLOSED.** 54 step 1 (#29), P0a gate half (#30), and P2 cortex companion (#159) all merged: an agent through the gate fetches findings with lineage, adjudicates/overrides, and the deposit lands on the cited atoms (`overrideCount:1`), tenant-scoped and rail-quiet. The decoupling bite is resolved. Chain: ~~54 step 1~~ → ~~P0a+P2~~ → 56 step 5 (now only needs the gate-front seam, 54 step 2). Pre-mortem cleared green 2026-06-09.
