...
- **P0b, canonical atom-id key (now-fix, load-bearing).** Id key-spaces diverge (`code_atoms.id` UUID vs `did:hauska:code-section:{id}` vs `reasoning:`/`websearch:`), so the overlay silently misses on the HTTP path too. One canonical key function fixes it. Dispatch: [`_dispatches/2026-06-09_cc-agent-C_atomid_namespace_normalization.md`](_dispatches/2026-06-09_cc-agent-C_atomid_namespace_normalization.md). This is the part that bites at launch and is sequenced now.
...
Plus a generation-persists-citations acceptance test (prove the server-side job persists citations, not assume it). **Sequencing dependency:** P0b lands now; **P0a + P2 must precede [`56`](56_engine_extraction_sprint.md) step 5 (cut consumers to the gate)**, otherwise the flywheel goes silent the moment adjudication is routed through the gate. Pre-mortem cleared green 2026-06-09 (tenant-scoping condition on `codex_findings_fetch` folded into the P0a dispatch).
