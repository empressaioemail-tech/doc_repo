...
Plus a generation-persists-citations acceptance test (prove the server-side job persists citations, not assume it). **Sequencing (P0b done; 54 step 1 done):** sprint-54 step-1 gate tenant resolution merged (#29; `AuthContext` carries `jurisdiction_tenant`), so **P0a + P2 are now READY** and just **must precede [`56`](56_engine_extraction_sprint.md) step 5 (cut consumers to the gate)**, otherwise the flywheel goes silent the moment adjudication is routed through the gate. Chain: ~~54 step 1~~ → **P0a+P2** → 56 step 5. Pre-mortem cleared green 2026-06-09 (tenant-scoping condition on `codex_findings_fetch` folded into the P0a dispatch, keyed on `jurisdiction_tenant`).
