...
- **P0b, canonical atom-id key (now-fix, load-bearing).** Id key-spaces diverge (`code_atoms.id` UUID vs `did:hauska:code-section:{id}` vs `reasoning:`/`websearch:`), so the overlay silently misses on the HTTP path too. One canonical key function fixes it, placed in `@hauska/atom-contract` so both the HTTP path and the gate consume one source (the gate has no import path to `lib/codes`). Dispatch: [`_dispatches/2026-06-09_cc-agent-C_atomid_namespace_normalization.md`](_dispatches/2026-06-09_cc-agent-C_atomid_namespace_normalization.md) (cc-agent-AC defines, cc-agent-C consumes). This is the part that bites at launch and is sequenced now.
...
- **Briefing-source decision (open).** Briefing-source citations are outside the code-section ledger. Recommended for launch: document them as intentionally non-deposit for code calibration (code-section is the priority); expand to a separate briefing-source overlay class later if site-context calibration is wanted. Operator call.

Plus a generation-persists-citations acceptance test (prove the server-side job persists citations, not assume it). **Sequencing dependency:** P0b lands now; **P0a + P2 are downstream of sprint-54 step-1 gate tenant resolution** (the gate has no tenant field today, so the tenant-scoping acceptance cannot be built yet) **and must precede [`56`](56_engine_extraction_sprint.md) step 5 (cut consumers to the gate)**, otherwise the flywheel goes silent the moment adjudication is routed through the gate. So the chain is 54 step 1 → P0a+P2 → 56 step 5. Pre-mortem cleared green 2026-06-09 (tenant-scoping condition on `codex_findings_fetch` folded into the P0a dispatch). Confirmed HELD by the 2026-06-09 gate recon.
