...
**Substrate v1 status as of 2026-05-19 wind-down.** Sync 1 (atom contract published) DONE — `@hauska/atom-contract@1.0.0` on npm, v1.0.0 tag pushed. Sync 2 (adapter contract) DONE. Sync 3 (retrieval API contract) DONE. Sync 4 (first jurisdiction passes eval) DONE — Grand County partial (IRC R301 + IWUIC) passed 0.9 / 1.0 / 1.0 quality bar; Grand County extended to full coverage (290 atoms with LAND_USE added). Bump 1 cross-repo PRs landed: hauska-engine #1 (atom-contract-pin shim flip), hauska-mcp-server #1 (Stream 2A wiring against real retrieval API), hauska-engine #2 (Path B migration tool), hauska-engine #3 (Grand County Sync 4), hauska-engine #4 (Grand County LAND_USE); legacy-design-tools #27 (README hand-off pointer). All squash-merged. Sync 5 (20-jurisdiction quality-gated corpus) open; next session is Bastrop UDC via B3 publisher adapter against bastrop.gov (Bastrop UDC absent from legacy Neon since BASTROP BUILDING BLOCK / B3 CODE lives outside Municode). Allocation decision for Session B is operator-side per the 2026-05-19 wind-down hand-off; cc-agent-E or a new agent.

**Outstanding can-kick from 2026-05-19 doc-sweep.** legacy-design-tools api-server import migration from `@workspace/empressa-atom` to `@hauska/atom-contract` is the one named deferral; dedicated cc-agent session within 1-2 weeks; workspace-private path stays valid through the transition.
