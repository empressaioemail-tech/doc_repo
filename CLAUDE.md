...
**Ground-truth reconciliation (2026-06-06; supersedes the point-in-time numbers above).** Per the cross-repo recon at `_research/2026-06-06_cross_repo_recon.md` (six spine repos read against live main): the engine corpus committed snapshot (2026-05-26) is 34 jurisdictions / 21,126 atoms, all passing — far above the 698/4 figure above, which reflected the Sync 4.5 checkpoint only. Of the 34, exactly two are public-free (Bastrop 193 atoms on the B3 code edition, Grand County/Moab 285) and 32 are platform-internal; the public-free Layer-1 catalog is therefore roughly 478 atoms across 2 jurisdictions. Any external-facing figure must carry that public-vs-internal split, never a bare headline. The sourcing posture of the 32 platform-internal jurisdictions was not established by the recon and is not asserted here. The atom contract is renamed **`@empressaio/atom-contract`** (2026-07-06 branding decision; source `package.json` verified 2026-07-20) and is published through **v1.22.0 on npm** (verified live 2026-08-20; sixteen minors between 2026-07-07 and 2026-08-12, and the `v1.7.0` figure this doc carried until then was fifteen versions stale. ADR-025 covers 1.7.0 and ADR-028 proposes 1.8.0; **1.9.0 through 1.22.0 have no ADR at all**, and the `./property`, `./reasoning` and `./testing` subpaths are undocumented. ADR-028 shipped exactly one of its six field groups, the verified-absence pair, while still marked `proposed`. The older `@hauska/atom-contract` name is frozen at 1.6.1 and appears in historical records only); `accessPolicy` is a five-value union (`public-free`, `public-paid`, `platform-internal`, `tenant-private`, `tenant-shared` — the fifth added in 1.2.0), not the four-value union recorded at Sync A above. Two atom families shipped beyond the data/skill/execution/actor/intent framing: encumbrances (ADR-020/021) and workspace. The Hauska MCP server exposes **63 tools across four gates (public/codex/reporting/map) on deployed main as of 2026-07-15** (live introspection: public 6 / codex 5 / reporting 46 / map 6) — the four-gate rework merged and deployed; the earlier "59 tools / three gates (public/codex/cortex)" figure is superseded (`cortex` gate retired in favor of `reporting`, `map` added). Verify live via the admin introspection endpoint before quoting. Gated by product key at call time (malformed or unknown keys 401; only the no-header anonymous path resolves to public). The 2026-06-06 "46 tools" figure this paragraph originally carried was itself stale; corpus/tool/contract counts must be traced to live `gh`/`npm`/`gcloud` state, not to this doc. **Store scale, last measured 2026-08-31:** `hauska_mcp.atoms` is 111,241,840 reltuples over 192 GB. The prior reading was 100,025,152 over 131 GB on 2026-08-20, itself roughly double the 43.6M in the 2026-08-12 forensics. The store grew about 11.2M rows and 61 GB in eleven days, so any figure here is stale within days by construction: re-measure, never quote. Ten shipped contract fields are at zero in a 1,025-row sample. Evidence: `_inbox/2026-08-20_store_audit_atom_graph.md`. **Every number in this paragraph is a landmine a fresh agent will quote. Treat each as a claim about its stated date and re-run the instrument before using it.**
