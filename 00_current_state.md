...
- **cc-agent-C** → legacy-design-tools — Codex Phase 2 reviewer-surfaces dispatch delivered in full: CDX-3 (#69) and CDX-4 (#70) merged, CDX-5 (#71) open and awaiting merge. Dispatch complete on #71's merge; cc-agent-C idle, queue empty.
...
- **Teed-up cc-agent dispatch queue** (2026-05-21, updated 2026-05-22). cc-agent-C: Codex Phase 2 reviewer-surfaces dispatch fully delivered — CDX-3 (#69) and CDX-4 (#70) merged, CDX-5 (#71) open and awaiting merge. cc-agent-C is idle with an empty queue; next dispatch is a planner call (Phase 2 still has CDX-QA-1 as a planner deliverable, CDX-9 gated on DA-5; or a different workstream). cc-agent-AC: QA-17 (#64), api-server import migration (#65), and the `@workspace/empressa-atom` retirement (#67) all merged; dispatch queue empty, no current dispatch. QUEUED, not yet fireable: ECI atomization P1 ([`_dispatches/2026-05-21_eci_atomization_p1_registry_scaffold_QUEUED.md`](_dispatches/2026-05-21_eci_atomization_p1_registry_scaffold_QUEUED.md)) — gated on Nick creating the `empressa-atom-internal` repo and a cc-agent seat freeing. QA-20 routed into cc-agent-E Lane E Phase E1.
