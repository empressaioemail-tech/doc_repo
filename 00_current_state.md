...
- **cc-agent-C** → legacy-design-tools — Cortex QA close-out + QA-22 Part 1 + codex-reviewer-qa scaffold all merged (PRs #59-63, #66); cleanup batch open as PR #68 (CI green, awaiting merge). Next: Codex Phase 2 reviewer surfaces (CDX-3/4/5), dispatched and gated on #68 merging.
...
- **cc-agent-AC** → legacy-design-tools (own clone `P:\ldt-ac-qa17`) — QA-17 (#64) and api-server import migration (#65) merged; `@workspace/empressa-atom` retirement open as PR #67 (CI green, awaiting merge), which completes the ADR-018 transition. Dispatch queue empty once #67 merges.
- **Nick** — merge, deploy, decisions.

## 5. Recent sessions

- 2026-05-21 cc-agent-C — legacy-design-tools cleanup batch (PR #68, open): QA-15 plan-review header bell (14 call sites), QA-26 root `.gitattributes`, flaky `lib/codes` CI fixed (30s testTimeout on 3 files).
- 2026-05-21 cc-agent-AC — `@workspace/empressa-atom` retirement (PR #67, open): migrated the final two consumers, removed the staging package (34 files, 3343 deletions); completes the ADR-018 transition for legacy-design-tools.
...
- **Teed-up cc-agent dispatch queue** (2026-05-21). cc-agent-C: cleanup batch delivered as PR #68 (open, CI green, awaiting merge); Codex Phase 2 reviewer surfaces ([`_dispatches/2026-05-21_cc-agent-C_codex_phase2_reviewer_surfaces.md`](_dispatches/2026-05-21_cc-agent-C_codex_phase2_reviewer_surfaces.md)) is dispatch 2, not started, gated on #68 merging. cc-agent-AC: QA-17 (#64) and api-server import migration (#65) merged; the `@workspace/empressa-atom` retirement is done — PR #67 open, CI green, awaiting merge. cc-agent-AC's dispatch queue is empty once #67 merges. QUEUED, not yet fireable: ECI atomization P1 ([`_dispatches/2026-05-21_eci_atomization_p1_registry_scaffold_QUEUED.md`](_dispatches/2026-05-21_eci_atomization_p1_registry_scaffold_QUEUED.md)) — gated on Nick creating the `empressa-atom-internal` repo and a cc-agent seat freeing. QA-20 routed into cc-agent-E Lane E Phase E1.
...
- **Flaky `lib/codes` CI timeouts** — **fixed in PR #68** (open). The `queue`/`orchestrator`/`bootstrap` test files each set a 30s `testTimeout`; the other five `lib/codes` files keep the strict 10s. Drop this item once #68 merges.
