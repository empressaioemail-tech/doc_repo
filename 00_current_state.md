...
- **Doc-set staleness gap, pre-courier-protocol work** (surfaced 2026-05-22). Lane C.4 shipped 2026-05-20 (legacy-design-tools PRs #46/#51) but `42_design_accelerator_program_plan.md` still called it "the remaining build" in its 2026-05-21 edit, and the planner re-dispatched already-done work off it. The `_inbox/` courier protocol began 2026-05-21; cc-agent work merged around 2026-05-19/20 may not have reached the doc set. A reconciliation pass over `42` and the other sprint docs against the legacy-design-tools merged-PR history is owed before trusting any "remaining" or "gated" status in them. Planner discipline: verify a stream's status against the live repo (`gh pr list`) before dispatching or re-activating, not against the doc set alone.
