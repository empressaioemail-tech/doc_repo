...
- RULED 2026-08-03 (operator): **cert-with-scope-annotation.** A cert ISSUES with a declined core rail, carrying a per-rail annotation naming what happened (the named decline reason) and what it takes to fix (the defect class it joins on the backlog); class-fix + re-warm upgrades the cert in place. A cert with zero annotations is a full cert. Coverage statements derived from annotated certs must carry the annotation, never a bare headline. Decision record: [`_decisions/2026-08-03_cert_scope_annotation_ruling.md`](../_decisions/2026-08-03_cert_scope_annotation_ruling.md) (reversal criterion: annotated certs consumed downstream as full certs).
