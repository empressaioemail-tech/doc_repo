...
| **Two zoning registries disagree about coverage, and one parcel's answer depends on which is asked.** `hauska-engine`'s `zoning-staging` registry and `legacy-design-tools`' `zoning-layers` registry cover different parcel sets for the same city. CTX-STAMPFALL ships a stamp-sourced `value` fallback so 17 Bastrop and 50 Travis parcels can leave `unaccounted`; that is honest provenance but it is not coverage, and the two registries still disagree | `_dispatches/2026-09-09_ctx-stampfall_dispatch.md`; flag pinned in code by test in `hauska-factory`; root established by CTX-STAMP 2026-09-09 | **Instrument-readable:** deferred by operator 2026-09-09 to unblock the CTX bake. Revisit when the six counties serve in production, or sooner if a third registry appears or a stamp-sourced `value` is found stale in production. NOT judgment-readable |
