...
| F20 | The operator, 2026-09-12: "when I search a property and make the property subject so the program clicks the property, versus me manually clicking a property, when the system selects it the setbacks don't show up and I have to click another property then come back and click my subject in order for the setbacks to show." Two placement paths for one parcel produce two sealed sheets, and only the click path carries the setbacks row; invariant I2 (the card reads the sealed sheet and issues no lookup) is honoured by both, so the difference is in what each path seals. Mechanism believed: the search-landing path seals without the live-envelope augmentation #386 wired into `resolveSheet`, or seals before the record point exists; alternative: a sheet cache keyed by parcel id holds a sheet sealed on incomplete inputs until a click on another parcel evicts it. Owner: P-174. | operator report and screenshot 2026-09-12; P-153 cp2 on `augmentFacetsWithLiveEnvelope` and the `!seed` gate. |
| F21 | The record-served Property Explorer facets payload (`X-Pe-Read-Path: record`, P-152 lane 2 onward) carries `cityLimitsFact` without `queryPoint` on every parcel read 2026-09-13 (48021:34049, 48453:113408, 48209:97658), while the MCP snapshot carries it. The probe's near-bbox and ring legs depend on that point and read UNMEASURED on P-158 since the cutover (the planner hand-probed P-158 for that reason). P-151 ruled that placement seeds from the record point; a search-landed subject reading the record path has no point to seed from while a clicked subject has the click, which is a third candidate mechanism for P-174 beside the lane's cache mechanism. Owner: the wave-3 planner (P-174 lane to test; P-152 lane 3 to restore the point on the record path). | overseer reads 2026-09-13, `_inbox/2026-09-13_p175_overseer_review.md` |
| F22 | `hauska-engine-api` (hauska-prod-497015, us-central1) is invocable by `allUsers` (`roles/run.invoker`) with ingress `all`, and no `ENGINE_API_GATE_TOKEN` secret exists in either GCP project, so the engine's bearer check is a no-op; the P-152 lane-5 composer gate is defence in depth for the two known callers and does not stop a caller who sets `x-hauska-access-tier` against the public URL. Owner: P-179 (mint, mount, verify by violation, then IAM). | wave-4 planner (Q5), confirmed by the overseer with `get-iam-policy` and `secrets list` 2026-09-13 |
| F23 | The feasibility export's refresh hop produces no new document: two `export_instrument` feasibility calls for `48021:34049` a minute apart (2026-09-13 23:00Z) returned byte-identical PDFs whose `/CreationDate` is 19:12:15Z, two hours before `hauska-engine-api-00217-gir` deployed; the P-167 lane saw the same across repeated calls. P-155's predicate (202 with a job reference, download within the poll budget) held on 2026-09-12 and does not hold now. Mechanism unknown (cached 200, silent job failure, or the MCP skipping the refresh hop); no Cloud Run job named for feasibility exists in either project. Owner: P-155 reopened, lane `p155-refresh`. | overseer live exports 2026-09-13; P-167 wave-5 close |
| F24 | The City of Bastrop's two GIS layers disagree on 1109 Pecan (SF-1): One Click (`Parcels_One_Click/23`, edited 2026-08-24) 25/5/25/15 citing Ordinance 2019-51 on draft zone types; Zoned Parcels (`Zoned_Parcels/83`, edited 2026-07-23) 30/10/30/20 citing BDC Sec. 14.02.003. Our card follows the ordinance text (30/10/30/20, effective 2026-04-14, human-verified) and matches Zoned Parcels, and says nothing about the One Click card a customer can open. R-1: a conflict row. Owner: P-154 wave 6; the operator asks the city which card is current. | overseer live reads 2026-09-14 RESOLVED 2026-09-14 (A-148): the city's authoritative layer `Zone_Types/25` (updated 2026-07-09) says 30/10/30/20 in its text fields and 25/5/25 in its unrefreshed numeric shortcut columns; One Click reads the numbers. One source, stale columns. |
...
| 16 | P-174 | ONE PLACEMENT PATH | hauska-map | F20: one sealing function for every entry path with one input shape (P-151); the branch or cache that seals without setbacks deleted; a test that fails on the bug. | A search for `1109 PECAN ST` lands on a card showing setbacks with no second click (operator observed). | none; wave-3 addendum |
