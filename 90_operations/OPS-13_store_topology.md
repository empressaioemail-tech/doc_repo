...
Note for anyone planning a geometry query: `neondb` has **no PostGIS**. `select ... st_xmin(geom::geometry)` fails with `type "geometry" does not exist`. Geometry is stored as `jsonb` in a `geometry` column, alongside four precomputed bounding box columns, `west_lng`, `south_lat`, `east_lng`, `north_lat`. Those bbox columns are plain doubles and are the cheap way to ask a geographic question. This matters more than it looks, and the next section explains why.
