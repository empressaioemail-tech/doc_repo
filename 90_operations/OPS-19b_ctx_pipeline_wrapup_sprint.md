...
| 9 | overlayDistricts/maxImperviousCoverPct writers never emit a `not-applicable` cell state — **CORRECTED 2026-09-05: data-layer fix confirmed real, serve-layer fix was NOT done, previously marked Closed in error.** PR #87 merged with zero live effect at first (caught by the Factory lane); integration seat found both jobs running stale images, rebuilt, ran the real `--apply` for all 6 counties (overlayDistricts) and Travis (maxImperviousCoverPct) — live-verified 0 unaccounted for overlayDistricts, matching the full 981,405-parcel universe. **But this only fixed the store.** Follow-up research found `factory-publish-gate-sched`'s `SLATE_1_RAIL_KEYS` hard-codes 5 rails that do NOT include either of these two, and the hourly scheduler passes no `--rail` override — so no gate verdict was ever being computed for them, and per `parcelRecordAllowlist.ts`'s own rule ("no verdict → legacy") real traffic kept serving legacy regardless of the correct data underneath. A one-time manual catch-up gate run was executed to land a real verdict now; a permanent fix (so the hourly job covers these rails going forward, without breaking `maxImperviousCoverPct`'s Travis-only scope for the other 5 counties) is dispatched to the Factory lane, not yet done. **Do not mark this item Closed again until the permanent gate-slate fix lands and a live customer-facing request is confirmed serving `record`, not `legacy`, for both rails.** | hauska-factory | Not closed — gate-slate fix pending |
