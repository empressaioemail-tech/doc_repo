...
| 9 | ~~overlayDistricts/maxImperviousCoverPct writers never emit a `not-applicable` cell state~~ — **DONE 2026-09-05.** PR #87 merged but had zero live effect (Factory lane's own live-verification caught this: 0 not-applicable cells existed in production hours after merge — the fix only fires on a real `--apply` job execution, which the lane correctly could not trigger itself, LAPTOP_WRITE_FROZEN). Integration seat found both jobs were also running stale images (built 2026-09-03/09-02, before the fix merged) — rebuilt via `cloudbuild.parcel-acquire-gis.yaml`/`cloudbuild.parcel-wave2.yaml`, then ran the real `--apply` for all 6 counties (overlayDistricts) and Travis (maxImperviousCoverPct, Austin-watershed-scoped by design). Live-verified after: overlayDistricts now 204,194 value / 263,417 absent-verified / 513,794 not-applicable / **0 unaccounted** — exactly the full 981,405-parcel universe. maxImperviousCoverPct: 22,139 value / 114,109 not-applicable (up from 0), remaining 845,157 unaccounted is the other 5 counties this Travis-only rail never targets, not a gap. | hauska-factory | Closed |
