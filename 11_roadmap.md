...
- [ ] **Cutover env-var rebind — Track B (rotation-pending restore)** — 13 vars awaiting vendor coordination via Monday Bastrop message: Spireon×3 (Solera Tier-2), Verkada×2, ESRI/ArcGIS×3, Calendar API key×1 (BeWith, post F-7/F-8), VFD codes×6 (Bastrop VFD admin). Bind as fresh values arrive. Verify integration health endpoints post-bind. Absorbs prior A.6.b / A.8.b rotation work. `security / follow-on / smartcity-os` · Nick + cc-agent · L.
