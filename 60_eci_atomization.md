...
ECI MCP is non-commercial per [`29_mcp_surface_tier_model.md`](29_mcp_surface_tier_model.md). Access control only via ADR-017 `platform-internal` scope; restricted to authorized internal actors (operator, internal strategic agents). No metering, no revenue routing, no take rate. Procedure-execution atoms (ADR-013) generated by ECI MCP calls are internal-tenant and not routed for revenue share. Separate endpoint (`mcp.internal.hauska.dev` or equivalent) recommendation from [§Open questions](#open-questions) below carried forward as design guidance; not a tier-model question.
