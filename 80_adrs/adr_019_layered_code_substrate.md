...
- **2026-06-09 (cortex-local reasoning layer sits in front of the immutable Layer-1 corpus):** the national code-warming sprint adds a cortex-local, web-first, mutable, confidence-bearing reasoning layer (`reasoning_atoms`) that grounds model-code references on demand and accretes from usage, distinct from the engine Layer-1 corpus this ADR governs. The relationship: read-time precedence is corpus (this ADR, licensed/structural, immutable), then the reasoning layer (web-first interim grounding), then live web-fetch. The reasoning layer does not change Layer 1; it is the demand-driven grounding cache in front of it, and it is where mutable confidence and arrow-two calibration live (the corpus stays rebuilt-immutable; calibration reaches corpus atoms via an id-keyed overlay, not mutation). The ICC and NFPA licensed cutover remains the upgrade that fills the public Layer-1 corpus and swaps the reasoning layer's deeplink display for licensed display. See [`57_national_code_warming_sprint.md`](../57_national_code_warming_sprint.md) and [`_decisions/2026-06-09_codewarm_arrow_two_combined.md`](../_decisions/2026-06-09_codewarm_arrow_two_combined.md).
