...
- **`cited-untracked` answers differently depending on the checkout.** It reports 113 tracked citations of untracked paths in the main checkout and 0 from a fresh worktree at the same commit, because a worktree does not carry untracked files. Exactly one of those 113 names a file created this session: `_catalog/tooling_register.md` cites `scripts/enforcement/harness-parity.mjs`, which is untracked until this lane's commit. The other R-06-adjacent hit is a May 2026 file citing the Claude harness's worktree directory, which git does not track, and is not this lane's. Worth noting that a control whose answer depends on which checkout it runs in is measuring the environment as well as the repo.
