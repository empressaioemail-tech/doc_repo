...
date: 2026-09-16
last_updated: 2026-09-17 (23:20Z, after the engine deploy landed and seven of eight lanes closed)
...
| 2026-09-17 23:20 | **The engine deploy landed and seven of the eight lanes closed. Two of them returned the decisions the program was waiting on.** **Engine deploy DONE and verified by field:** retrieval-api `00102-ciz` (tag `p260-72d72c0`, explicit revision at 100 percent — correct for the pinned service) and engine-api `00253-qan` (`latestRevision: true` at 100 percent), both from engine `72d72c02`. P-260's registry fix is on the customer path. **P-326 answered the setback question and it is far better than feared.** Of the 58,339 parcels refusing setbacks, **48,829 (83.7 percent) are servable by P-300's jurisdiction-default row** and only **9,510 need a real district table**. Travis, the biggest chunk, is 32,140 servable against 1,743 needing a table. The caveat that matters: for 48,825 of the servable ones **no zoning layer exists for their city either**, so the default line is the ONLY input that can ever set them. **This redirects the setback plan: fire P-300 broadly rather than building district tables.** **P-204 returned its ruling table and found a live customer defect.** All five rails recommend CUT OVER, but they are NOT one decision: group A (`parcelGeometry`, `pipelines`) is pure accounting — atom-backed, proven load-bearing by ablation, no writer to build and no change to what a customer reads. Group B (`etjStatus`) carries **a live defect in hauska-map: the BFF replaces `cityLimitsFact` wholesale, so the customer reads a false negative in both directions today** even though the determination exists on the authoritative path. Group C (`landUseDescription`) is **the only cutover that SUBTRACTS a customer-visible value**, gated on the Hays/TxGIO-CAD join-hold. Group D (`railCorridor`) has no serve path at all, so its choice is build-the-first-serve or retire a rail we ruled in on 2026-08-09 and have since written about 139k atoms for — and there the dispatch's own premise is false, because cutting over ADDS a customer outcome. **Nine PRs are open and NONE is merged**: map #417 (P-257) and #418 (P-270); LDT #717 (P-257), #718 (P-322), #719 (P-270); factory #173 (P-319/320/321), #174 (P-322 sweep), #175 (P-325), #176 (P-326). factory #173 is UNSTABLE on `ldt-sha-comment-presence`, which failed with `fatal: bad object` on its base ref — the check could not run, which is an infrastructure failure and not a finding. P-264 is the only lane still working. |
