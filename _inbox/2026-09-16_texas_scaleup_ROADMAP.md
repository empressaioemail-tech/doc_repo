...
| Hays production-only re-grade | Grades P-301 | `mxkft` refused `LEASE_HELD` at 15:14:27Z (nothing written). `hays-when-lease-free.sh` (session 06a91261's tab) polls every 60 s and runs it when the production store is free; gives up at 17:45Z. Status line: `P:/tmp/integration-handoff/REPUBLISH_IN_PROGRESS.txt` | Do not start it from another session while that line reads WAITING or RUNNING. Pass: walk `verdict: pass`, `declinedRetirement.accepted` 16, `byRefusalError.record_retired` 16 |
...
| P-266 with P-268 | Open trivial rails; must-pass refusals (factory #168) | **Close filed 14:34Z (partial) but the lane is still running** its dry-run check across the six counties. It holds the factory-store lease until 15:52Z and the production-store lease until 16:42Z | Review #168 once the dry-run check is in; run the writers per county, grade by the gate |
| P-275 | Live-currency checks for five counties (engine #467) | **Close filed 14:50Z (partial)**; the claim is live in its own worktree, not `P:/doc_repo` | Review; any reactivation needs the operator's authorisation |
...
| P-303, P-304, P-305 | Panel decline, anonymous area figure, key drift | **Fired** (claims at 14:53Z, 14:49Z; P-304's claim not found) | Review, merge, deploy as they land |
