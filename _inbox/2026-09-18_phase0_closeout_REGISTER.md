...
date: 2026-09-18
last_updated: 2026-09-19 (14:40Z, A-231: the landed wave merged and deployed; P-363 applied; earlier A-227 to A-230)
...
| P-300 with P-338 | Jurisdiction-default line where the city's ordinance sets one (A-224); district-miss and zoning-not-acquired refusals name the district or city | corpus, hauska-factory, surfaces | 1, 2 | 2 | **compiled 2026-09-18 in two halves:** writer `_dispatches/2026-09-18_p300-p338-setback-residual-writer_dispatch.md`, surfaces `_dispatches/2026-09-18_p338-refusal-surfaces_dispatch.md`; population 58,339 (9,510 district misses, 48,829 in 40 no-layer cities). **2026-09-19: classification closed (40 cities reconcile exactly; 8 unclassified, 3,195 parcels); the writer build MERGED on an explicit instruction the lane received (factory #192 `be295755`, corpus #12 `1448586`, `@empressaio/setback-corpus@1.5.0` published 13:13Z; factory main pins 1.5.0).** Reviewed after the fact by the seat: acceptable, with P-377 carded (the job never reopens its own refusals and checked absences). The dry run (the lane handed it off) is the seat's: images `08412cea` / `1a00d747` from `be295755`, dry run `tpsx7` running. Record `_inbox/2026-09-19_a231_merges_etj_deploys_RECORD.md` |
...
| P-354 | Georgetown's adopted rewrite is served (A-218) and every surface names its adoption and effective dates, never "vintage unknown"; a rule row carries both dates as fields | setback-corpus, legacy-design-tools, hauska-factory, hauska-map | 2 | **now (A-218: compile ahead of Wave 2)** | **closed-partial (lane). Map #421 (`b1b67641`) and LDT #720 (`2e7ca7c4`) MERGED and DEPLOYED 2026-09-18, inert until row dates are served;** factory #179 waits to be read against P-363 and the P-300 writer; corpus #11 and **its publish (now 1.6.0: 1.5.0 was taken by P-300/P-338 on 2026-09-19)** go with the consumer pin bumps after #179, and **not before P-377 ships** (a pin move would otherwise freeze the new rows out of parcels already refused); the writer re-run for 48491 then P-350. Finding: `todayIso()` is the UTC date |
...
| When (UTC) | Change |
|---|---|
| 2026-09-19 14:40 | A-231. **Merged and deployed the landed wave:** P-270 (map #426, LDT #727), P-340 (map #427, LDT #729), P-339 (LDT #728), P-367 (factory #190); P-366 (#726) held on red CI. **P-359 live:** 0104 applied, the ETJ re-ingested (330/8/2/15, the lane's prediction exactly), cortex-api `00845-qev` serving after a pre-registered canary (every difference one of the four named changes), Property Explorer on `55613fad`; signed probe PASS 41 / FAIL 8 / UNMEASURED 10 (the eight are the draw-route family; the MCP leg passes on all). **P-363 applied** in five counties (257,027 parcels, graded PASS). **The writer build merged by its lane on an instruction it received; corpus 1.5.0 published; seat review after the fact: acceptable, P-377 carded.** P-376 carded (incorporated parcels read ETJ unresolved). Seat process violation recorded: the ETJ re-ingest ran inside P-368's heavy-scan window (the take's exit code was not checked); every write command now gates on it. |
