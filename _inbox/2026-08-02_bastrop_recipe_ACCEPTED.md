...
R35 EXTENSION (2026-08-03 — the SF-1 re-sweep found 5 promoted-but-cert-fail parcels; split into TWO classes): the 5 all have VALID served envelopes (R32 + facesAnswer pass on the STORED atom) but cert's RECOMPUTE (`computeWarmCandidateFromBoundary`) returns empty. They split:
- CLASS A — NULL-SITUS RE-PLAT SUCCESSORS (8741972/73/74; 34065 re-platted → these; situs = null): a re-plat successor with NULL situs has NO front-street reference, so orientation recompute is empty for the SAME reason as R35's landlocked case. R35 EXTENDS to cover null-situs parcels: HONEST-DECLINE the orientation (no determinable frontage — null situs), served as parcel+zoning+setbacks, a cert PASS (disclosed decline), never a fabricated front. This is R15 (re-plat successors) meeting R35 (no-frontage decline). Generalizes: re-plat successors with unpopulated situs are common at scale.
- CLASS B — VALID-SITUS RECOMPUTE-EMPTY (28855 = 509 Laurel, 30857 = valid situs + valid envelope): these have real frontage and a valid stored envelope, but cert's boundary RECOMPUTE returns empty. This is a genuine warm/cert RECOMPUTE divergence R33 didn't cover (R33 shared the MEASUREMENT; the CANDIDATE re-derivation still diverges). DO NOT "fix" this by making cert trust the stored atom over the recompute — that violates R10 (persisted==recompute; the persisted!=recompute defect fought all phase). Instead DIAGNOSE why computeWarmCandidateFromBoundary is empty on a genuinely-buildable parcel and FIX THE RECOMPUTE so it re-derives the same candidate the warm did → then persisted==recompute holds and cert passes for the RIGHT reason. R10 stays intact: cert never blindly blesses a stored atom over a fresh recompute.
