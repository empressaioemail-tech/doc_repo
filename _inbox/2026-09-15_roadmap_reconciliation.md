...
sources:
  - smartcity-dashboards origin/main 423450c0 (PR #75, `g153`, merged 2026-09-18) and smartcity-os origin/main 85a332e0 (PR #60, `g162`, merged 2026-09-18), read by the planner via gh the same day
...
| `g164-design-repairs` | G-164 | **CARDED 2026-09-18 (A-159), NO DISPATCH YET.** The 11 findings `g148` disclosed, now ONE row because they are one class of defect (a claim checked against source that went stale, mostly a stale negative) and because repairing a design and proving it are one unit of work. **This row is what turns the design gate green:** since A-159, R3 runs each design's instrument, so the gate exits 1 until these land. Ten are unblocked; the flood-study rainfall-atlas finding is entangled with G-125's broken `parsePfdsDepthTable` and carries that half blocked | carded from A-158's disclosure; the four failing instruments are `plan-review-departments`, `smartcity-flood-study`, `smartcity-map-dock` and `smartcity-overview-lens`, all exiting 1 on the shipped boards as re-read at 21:15:53Z. **Needs a decision: compile it as a lane, or work it at the planner seat, since the repairs are `doc_repo` design work and doc_repo commits are planner-owned** |
...
| The next lens build | ~~G-161 and D-13's dashboards change merged~~ **G-161 AND D-13'S CODE LANDED 2026-09-18.** G-161's fix is in `smartcity-dashboards` `main` `7487d7c0`; D-13's repoint is in the same commit and its production half is done too (A-10). **G-153 has since closed and merged (PR #75, `423450c0`), so G-152 IS THE NEXT LENS and is unblocked; G-151 and G-149 queue behind it.** The only constraint is one dashboards lane at a time |
...
| **Compile `g164` as a lane, or work it at the planner seat?** | the design gate, and therefore M5. It is 11 design repairs across four RATIFIED or APPROVED designs in `doc_repo`, which is planner-owned, so it can be either a lane that leaves uncommitted edits or planner work. Ten are unblocked; the flood-study rainfall-atlas finding carries a half blocked on G-125's broken `parsePfdsDepthTable` (OPS-17 `A-159`) |
...
| **FIRE THE TWO COMPILED DISPATCHES, NOW UNBLOCKED** (`g152-public-works-fire-ems`; `g163-opaque-platform-routes`) | M3's lens queue and the two opaque platform routes. **Both conditions are cleared:** PR #75 (`g153`) and PR #60 (`g162`) are MERGED, at `423450c0` and `85a332e0`, so neither branches onto a main missing the lane before it, and the `smartcity-os` intent-row posture that was blocking `g163` is corrected (A-160). The two lanes are in DIFFERENT repos, so they may run in parallel; `g152` blocks `g151` and `g149` behind it, and `g164` is not a lane and does not queue against them |
