---
id: 2026-08-25_county9_blanco_48031_handoff
title: Handoff — Texas leftover county 9 is Blanco 48031
date: 2026-08-25
status: ready_after_bexar_keep
plan_row: P-78
...
Bexar 48029 KEEP is filed. This child writes one leftover county. Quote `--vintage`. One FIPS. Then stop. Do not spawn. Do not commit.

---

You are writing one leftover county onto `cad_property`. The county is **Blanco 48031**. Not Bexar rewrite. Not Bandera. Not Atascosa. Not Bastrop. Not Caldwell. Not Hays. Not Williamson. Not Travis. Not Dallas. Not Tarrant. Not Burnet.
...
L17 is the inspect vintage pin, not a stop. Leftover still writes. Registry row `tx-48031` has `current_tax_year` null and `current_tier` null. Do not write a declared vintage onto the registry. Do not flip L17.
...
- Already written. Do not rewrite: Caldwell 48055 @ 2025 n=24989. Bastrop 48021 @ 2025 n=77799. Hays 48209 @ 2025 n=172116. Williamson 48491 @ 2025 n=282570. Travis 48453 @ 2025 n=380918. Atascosa 48013 @ 2025 n=34649. Bandera 48019 @ 2025 n=32755. Bexar 48029 @ 2025 n=703258. Confirm those from `_inbox/2026-08-25_leftover_queue.md` before you write.
...
5. `_inbox/2026-08-25_review_bexar_48029_leftover.md` (must be KEEP)
6. This file
...
Copy `scripts/fixtures/cad-ingest-apply-gate/packet.template.json` to `_inbox/2026-08-25_p78_announce_blanco_48031_packet.json`.

## Packet rules

1. FIPS is `48031`. `countyCount=1`. `secondCounty=false`.
2. All-year census, no `tax_year =` filter. Literal `'48031'`. File leftover year and declared L17 year even if n=0.
...
```
node scripts/cad-ingest-apply-gate.mjs --check --packet _inbox/2026-08-25_p78_announce_blanco_48031_packet.json
```
...
Announce `_inbox/2026-08-25_p78_announce_blanco_48031.md` before the upsert log starts. Keep after-counts out of the pre-apply announce. Isolated LDT at `46e1a5a1`. `--county=48031` only. No fallback flag.

Quote `--vintage` in PowerShell:

```
pnpm --filter @workspace/cad-ingest stratmap-landuse -- --county=48031 "--vintage=tier:stratmap-roll;adapter:stratmap;drop:stratmap25-landparcels_48031_lp"
```

Confirm the drop name from the zip / CLI. File before JSON, apply log, after JSON. Count prop_id `0` and do not call it success.

If leftover year equals a declared store year that inspect would read, probe one Blanco inspect parcel. If leftover year is off that year, or L17 is undeclared, skip the inspect probe and say why.

Then stop. File `_inbox/2026-08-25_p78_blanco_48031_leftover_close.json` with leave_behind. Do not commit.

## Do not

48029 rewrite. 48019. 48013. 48021. 48055. 48209. 48491. 48453. 48113. 48439. P-80. 280238 overlay. L17 / registry flip. Atoms `--apply`. Rematerialize. Gold 2800 restore. Memory pin raise. A second county in the same session.
