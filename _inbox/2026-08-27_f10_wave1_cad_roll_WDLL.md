...
- 2026-08-28 (A-016, from CP3): the conformant run chunks a county (ordered landing slices of about 50,000 rows, a run_event per chunk, the lease held for the whole county, stages B to E per chunk, idempotent stage and merge) so memory is bounded by the chunk; criterion 4 grades on Bexar clean through chunks with chunk size and peak memory filed. Criterion 3 grades only on a field-level explanation of the 48055 replay divergence and a fix or a named class whose disposition is not re-run. Criterion 6 is filed from 4j6pr before the next execute. Criterion 5 runs once, after chunking.
- 2026-08-28 (A-020, course correction): after criterion 5 closes, wave 1 is the Central Texas six only, `--counties=48021,48453,48491,48209,48055,48309`; the Travis join (P-80) and Hays landuse classes are dispositioned in the register before or during that run (fix, or an accepted degraded disposition the manifest shows); the lane reports whether Williamson and McLennan have landing. The 25-county remainder, the concurrency lever, and F-09 wait. The build-config PR (A-019) still precedes the run.
- 2026-08-28 (A-017, from 4j6pr distribution): wave 1 is the **31 counties with landing** that are execute or idempotent (25 + 6); the **217** `TX-LANDING-ABSENT` rows are absent sources, not loop defects, and read **lookup-failed with scope** on the manifest; the **F-09 acquisition card** is compiled by the planner when CP3 closes.
