---
id: 2026-08-25_county23_kaufman_48257_handoff
title: Handoff — Texas leftover county 23 is Kaufman 48257
date: 2026-08-25
status: ready_after_karnes_keep
plan_row: P-78
...
Karnes 48255 KEEP is filed. This child writes one leftover county. Quote `--vintage`. One FIPS. Then stop. Do not spawn. Do not commit.

Kaufman leftover 2025 may be on inspect. Still one county. Do not start Kendall.

---

You are writing one leftover county onto `cad_property`. The county is **Kaufman 48257**. Not Karnes rewrite. Not Johnson. Not Kendall. Not Dallas. Not Tarrant.
...
L17 is the inspect vintage pin, not a stop. Leftover still writes. Registry row `tx-48257` declares **2025 / stratmap-roll**. Re-verify. If leftover year equals that declared year, leftover is on the inspect read set. Set `inspectReadSet=true`. Still `willFlipL17=false`. Do not flip L17.
...
5. `_inbox/2026-08-25_review_karnes_48255_leftover.md` (must be KEEP)
6. This file
...
Copy `scripts/fixtures/cad-ingest-apply-gate/packet.template.json` to `_inbox/2026-08-25_p78_announce_kaufman_48257_packet.json`.

## Packet rules

1. FIPS is `48257`. `countyCount=1`. `secondCounty=false`.
2. All-year census, no `tax_year =` filter. Literal `'48257'`. File leftover year and declared L17 year even if n=0.
...
4. Declared is from live registry, re-verified. Hint (re-verify): `tx-48257` is 2025 / stratmap-roll. If leftover year is 2025, `inspectReadSet=true` and `willFlipL17=false`. If leftover year ≠ declared, `inspectReadSet=false` and `willFlipL17=false`.
...
```
node scripts/cad-ingest-apply-gate.mjs --check --packet _inbox/2026-08-25_p78_announce_kaufman_48257_packet.json
```
...
Announce `_inbox/2026-08-25_p78_announce_kaufman_48257.md` before the upsert log starts. Keep after-counts out of the pre-apply announce. Isolated LDT at `46e1a5a1`. `--county=48257` only. No fallback flag.

Quote `--vintage` in PowerShell. Set `NODE_OPTIONS=--use-system-ca`.

```
pnpm --filter @workspace/cad-ingest stratmap-landuse -- --county=48257 "--vintage=tier:stratmap-roll;adapter:stratmap;drop:stratmap25-landparcels_48257_lp"
```

Confirm the drop name from the zip / CLI. File before JSON, apply log, after JSON. Count prop_id `0` and do not call it success.

If leftover year equals declared L17, probe one Kaufman inspect parcel on smartsite.cloud and file structuralFact yearBuilt / livingAreaSqft / landAcres. Do not use `48021:34137`, `48453:280238`, `48029:262160`, `48085:10000`, `48091:10003`, `48121:10`, or `48187:106109`. If leftover year ≠ L17, skip the inspect probe and say why.

Then stop. File `_inbox/2026-08-25_p78_kaufman_48257_leftover_close.json` with leave_behind. Do not commit.

## Do not

48255 rewrite. 48251. 48231. 48221. 48187. 48171. 48163. 48149. 48139. 48121. 48091. 48085. 48053. 48031. 48029. 48019. 48013. 48021. 48055. 48209. 48491. 48453. 48113. 48439. 48259. P-80. 280238 overlay. L17 / registry flip. Atoms `--apply`. Rematerialize. Gold 2800 restore. Memory pin raise. A second county in the same session.
