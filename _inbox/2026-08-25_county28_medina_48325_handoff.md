---
id: 2026-08-25_county28_medina_48325_handoff
title: Handoff — Texas leftover county 28 is Medina 48325
date: 2026-08-25
status: ready_after_llano_keep
plan_row: P-78
...
Llano 48299 KEEP is filed. This child writes one leftover county. Quote `--vintage`. One FIPS. Then stop. Do not spawn. Do not commit.

---

You are writing one leftover county onto `cad_property`. The county is **Medina 48325**. Not Llano rewrite. Not Lee. Not Parker. Not Dallas. Not Tarrant.
...
L17 is the inspect vintage pin, not a stop. Leftover still writes. Registry row `tx-48325` has `current_tax_year` null and `current_tier` null. Do not write a declared vintage onto the registry. Do not flip L17.
...
5. `_inbox/2026-08-25_review_llano_48299_leftover.md` (must be KEEP)
6. This file
...
Copy `scripts/fixtures/cad-ingest-apply-gate/packet.template.json` to `_inbox/2026-08-25_p78_announce_medina_48325_packet.json`.

## Packet rules

1. FIPS is `48325`. `countyCount=1`. `secondCounty=false`.
2. All-year census, no `tax_year =` filter. Literal `'48325'`. File leftover year and declared L17 year even if n=0.
...
```
node scripts/cad-ingest-apply-gate.mjs --check --packet _inbox/2026-08-25_p78_announce_medina_48325_packet.json
```
...
Announce `_inbox/2026-08-25_p78_announce_medina_48325.md` before the upsert log starts. Keep after-counts out of the pre-apply announce. Isolated LDT at `46e1a5a1`. `--county=48325` only. No fallback flag.

Quote `--vintage` in PowerShell. Set `NODE_OPTIONS=--use-system-ca`.

```
pnpm --filter @workspace/cad-ingest stratmap-landuse -- --county=48325 "--vintage=tier:stratmap-roll;adapter:stratmap;drop:stratmap25-landparcels_48325_lp"
```

Confirm the drop name from the zip / CLI. File before JSON, apply log, after JSON. Count prop_id `0` and do not call it success.

If leftover year equals a declared store year that inspect would read, probe one Medina inspect parcel. If leftover year is off that year, or L17 is undeclared, skip the inspect probe and say why. Do not use `48021:34137`, `48453:280238`, `48029:262160`, `48085:10000`, `48091:10003`, `48121:10`, `48187:106109`, or `48257:10005`.

Then stop. File `_inbox/2026-08-25_p78_medina_48325_leftover_close.json` with leave_behind. Do not commit.

## Do not

48299 rewrite. 48287. 48265. 48259. 48257. 48255. 48251. 48231. 48221. 48187. 48171. 48163. 48149. 48139. 48121. 48091. 48085. 48053. 48031. 48029. 48019. 48013. 48021. 48055. 48209. 48491. 48453. 48113. 48439. 48367. P-80. 280238 overlay. L17 / registry flip. Atoms `--apply`. Rematerialize. Gold 2800 restore. Memory pin raise. A second county in the same session.
