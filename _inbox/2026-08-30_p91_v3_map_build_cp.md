...
## M-4 iframe multi-parcel canvas (returned third; supersedes the need for M-3's predecessor M1)

Read by the planner as a diff. Suite rerun by the planner: 373 of 373, up from 323.

The design premise the lane confirmed: rings plus per-parcel anchors compose in Web Mercator, so the cortex shared frame the v3 WDLL scoped as M1 is not needed and no shared origin is invented anywhere. Each parcel runs through the same `ringFit`, `ringPixel` and `groundPlan` the single-parcel view uses, so registration is inherited rather than reimplemented.

Attack 1: a canvas that quietly draws four of seven parcels. This is the defect class this operation keeps finding and it is invisible from outside. Answer: an undrawable parcel is named beside the canvas with its reason. Planner mutation, independent of the lane: silencing the truncation declaration fails 2 fixtures.

Attack 2: a set spread across a county stretches a tile mosaic into meaningless imagery. Answer: above a 5,280 ft extent the rings paint with no ground and the reason is stated. Planner mutation: collapsing the extent guard fails 2 fixtures.

Attack 3: the batch fan-out is unbounded. Answer: twelve anchors per batch, concurrent, with every row past the cap still carrying its own `anchorRead` and a top-level `anchorBatch` naming cap, received, attempted and notAttempted, so truncation declares itself rather than resembling absence. The node batch cap of 25 and the stub path are untouched.

Self-audit the lane ran and reported rather than hiding: its first mutation pass found ONE VACUOUS check, the Mercator scale correction, which is invisible at block separation because adjacent parcels are too close for the error to appear, plus two checks with only a single catcher. All three were closed with new fixtures before a second pass. Third lane in this program to audit its own checks; the vacuous one would have shipped silently.

Known gap, recorded by the lane as a passing fixture rather than left to be discovered: below two drawable parcels the panel falls back to today's single-parcel behaviour, which names none of the other rows, so a seven-parcel batch with one drawable parcel says nothing about the other six. That is a gap in the planner's brief, not the lane's work. Close it next: paint the undrawable list whenever the result carries more than one parcel, canvas or no canvas.

## Planner instrument failure, second of the day: a PR number asserted rather than read

The build chain's output was truncated with `tail -1`, so the planner never saw the PR URL its own `gh pr create` printed, assumed the next number in sequence, and attempted to merge #558. #558 belongs to another seat (`seat/property-ctx-p2b-serve`) and was already merged. The merge did not happen only because GitHub reported `mergeStateStatus: UNKNOWN` and the planner's merge guard requires `CLEAN`. The guard held; the reason it was tested is that a proxy was read instead of the record.

The real PR was #559, and it came back `BEHIND`: the other seat merged four commits while this wave built. Updated via `gh api update-branch`, CI re-ran against the moved base, and the merge went in on `Test` and `Typecheck` conclusion strings with `CLEAN`, as `bb814f1c`.

Both planner failures today share one shape, and it is already written in the doctrine: read the authoritative record, never a proxy for it. The tag instead of the digest this morning; a guessed PR number instead of the printed URL this evening. The repair in both cases is the same and is mechanical rather than attentional: do not truncate the output of a command whose identifier you will act on.

## M-3 iframe paint channel

(not dispatched; the multi-parcel canvas took priority after the operator's walk. Still ruled in with its two invariants.)
