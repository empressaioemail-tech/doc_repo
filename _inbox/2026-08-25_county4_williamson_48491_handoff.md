---
id: 2026-08-25_county4_williamson_48491_handoff
title: Handoff — Texas fill county 4 is Williamson 48491
date: 2026-08-25
status: filed
plan_row: P-78
...
You are writing one leftover county onto `cad_property`. The county is **Williamson 48491**. Not Bastrop. Not Caldwell. Not Hays. Not Travis. Not Dallas. Not Tarrant.

Pin is live on `P:/doc_repo` `main` `9753b83`. `--check --packet` must include `ldtSha` exactly `46e1a5a1`. A full 40-char SHA fails.
...
- Integration `P:/doc_repo` `main` `9753b83`. Declare your own seat, worktree, branch, commit.
...
- Already written. Do not rewrite: Caldwell 48055 @ 2025 n=24989 (Path B, L17 2026). Bastrop 48021 @ 2025 n=77799 (Path A plus 726 inserts, L17 2025). Hays 48209 @ 2025 n=172116 (Path A plus 40870 inserts, L17 2026, acres filled, year_built already saturated).
...
5. `_inbox/2026-08-25_review_hays_48209_leftover.md`
6. This file
...
Copy `scripts/fixtures/cad-ingest-apply-gate/packet.template.json` to `_inbox/2026-08-25_p78_announce_williamson_48491_packet.json`.

## Packet rules

1. FIPS is `48491`. `countyCount=1`. `secondCounty=false`.
...
```
node scripts/cad-ingest-apply-gate.mjs --check --packet _inbox/2026-08-25_p78_announce_williamson_48491_packet.json
```
...
Announce `_inbox/2026-08-25_p78_announce_williamson_48491.md` before the upsert log starts. Isolated LDT at `46e1a5a1`. `--county=48491` only. No fallback flag. TxGIO fetch is fine (`NODE_OPTIONS=--use-system-ca` if TLS needs it). File before JSON, apply log, after JSON (same all-year query). Count prop_id `0` and do not call it success.

If leftover year equals declared L17, probe one Williamson inspect parcel on smartsite.cloud and file the structuralFact yearBuilt / livingAreaSqft. Do not use `48021:34137`.

Then stop. File `_inbox/2026-08-25_p78_williamson_48491_leftover_close.json` with leave_behind. Review is the next card.

## Do not

48021. 48055 rewrite. 48209 rewrite. 48453 / Travis CAMA. 48113. 48439. L17 flip. Atoms `--apply`. Rematerialize. Gold 2800 restore. Memory pin raise. Full-SHA `ldtSha`. A second county in the same session.

## Why Williamson

CAPCOG leftover remaining after Caldwell, Bastrop, and Hays. Not the gold FIPS. Not Travis (P-80 CAMA join held; leftover on Travis is county 5). Not bulk_primary.
