...
4. FOLLOW-ON (not in this build): the tile stays dormant until a "parcel-terrain-model" capability is added to @empressaio/cortex-client with mcpTools: ["generate_parcel_terrain_model"]. LOCATION RESOLVED 2026-07-15: @empressaio/cortex-client and @empressaio/cortex-tiles are AUTHORED in legacy-design-tools at packages/cortex-client/ and packages/cortex-tiles/ respectively, published to npm via ldt's .github/workflows/publish-packages.yml ("Publish Packages"). The earlier "unknown location" was because the search looked in hauska-map (the command center), where these packages are only CONSUMED as npm dependencies, not authored. The loop is the exact one already run this session for the stale-hydrology-banner fix (PR #250): edit packages/cortex-client/ in ldt to add the capability, bump the version, run the Publish Packages workflow, then bump the command-center's dependency. Until that capability ships, the TILE_COMPONENTS entry is wired but ALL_TILES won't surface it. NOTE: the local-override tile pattern this build used is still correct for command-center-specific tiles (LiveMapTile-class, behavior that should not live in the shared published package); packages/cortex-tiles/ is the home only for SHARED library tiles.
