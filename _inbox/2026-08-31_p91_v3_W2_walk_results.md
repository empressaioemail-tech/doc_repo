...
**4. Flood `zoneSubtype` never reaches the row.** Serve-side. All four parcels read `present`, but three sit in the 0.2 percent band and 709 Laurel is minimal hazard. Confirmed at the paint layer: there is no flood tint variant on the canvas, so two materially different findings render identically.
