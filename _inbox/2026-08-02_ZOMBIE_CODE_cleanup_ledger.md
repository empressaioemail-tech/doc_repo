...
|---|---|---|---|---|
| `bastrop-district-cert-grade.mjs` (14,345 B) | hauska-engine/packages/engine-core/scripts | The NEW cert harness the fleet built instead of extending the proven `block13-cert-grade.mjs`. Superseded by the generalized block13-cert-grade (roster param) per the corrected dispatch. | RETIRE (fold any genuinely-needed piece into block13-cert-grade first). | The corrected Phase C run has block13-cert-grade generalized to a roster + grading the city; then this is dead. |
| `bastrop-layer23-roster.mjs` | hauska-engine/packages/engine-core/scripts | The divergent COHORT/roster wrapper (roster-field / bbox selection) that pulled wrong-district parcels into the SF-1 sweep. Replaced by dominant-district (R26) cohort logic in the corrected path. | RETIRE or REWORK into the dominant-district cohort (if the loader plumbing is reusable, rework; else retire). | Corrected dispatch defines the dominant-district cohort; then the old roster-field selection is dead. |
