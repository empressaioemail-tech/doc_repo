...
# Third ruling set, operator 2026-08-30 (function inventory conversation)

The thirteen-function starter inventory is confirmed as the working set ("nothing missing at this point, good start"); nothing struck. Ranking axis ruled: NOT by user value; order the inventory from what we hold today (strongest) to the biggest gap. Resulting order, provisional where marked: 1 screening (usable, shipped); 4 flood (partial-strong; studies remain); 3 zoning (district plus citation; code depth is F9); 11 listing history (disclosed web-search fallback); 12 outreach (year built only); 5 can-I-build-X (honest refuse; envelope is data lane); 2 area (dead but the build is fully scoped, closes with v3 itself); 6 ownership and equity (UNMEASURED, rank firms after measurement 4); 9 resale value (UNMEASURED pieces); 7 distress (not held; clerk index is the unlock, shared with 6); 10 permits (not held, per-city sources); 8 rental (the biggest gap: not public record at all, a feed ruling); 13 pro forma (held, sits atop the others). The gap ledger orders itself on this axis. NUMBERING AUTHORITY (added 2026-08-30 after the canvas agent flagged the collision): the numbers used in this paragraph are the ORIGINAL inventory ids from the chat conversation; the v3 WDLL's ledger table renumbers 1 to 13 in this ruled order and IS the authoritative numbering. Cite the WDLL table, never these ids. Commit freeze noted 2026-08-30: no doc_repo commits until the other seat's lane closes, per the operator.
...
5. The gap ledger itself starts from a field inventory: every field the current twin serves (by reading the write paths, not output), then the wanted list from the lens conversation. MEASURED 2026-08-30: `_inbox/2026-08-30_p91_measurement5_field_inventory.md` (agent read of every serve and bake path at `8d94ddad`; planner spot-checked the tier-2 and vintage claims at source). Three consequences for the ledger. First, the cheapest depth wins are fields the bake already holds and the panel never serves: apn, acreage, countyFips and countyName, facetCoverage, the whole provenance block, and the entire stored Tier-2 FEMA flood facet (only its refusal crosses the wire). Serving these is cortex serialization work, no acquisition at all. Second, a defect list for the property seat: draw landUse reads `desc`/`taxYear` where the bake writes `description`/`vintage` (two key mismatches), `yearBuiltFromBake` reads keys no bake writes, absent reads for flood/well/specialDistrict drop `sourceVintage` so absent-verified is unreachable except pipeline present-outside, and the layer manifest is always empty by construction. Third, confirmed absences: no drainage producer exists anywhere (grep exit 1 across both bakes, assembly, join, and CLIs), and zoning `codeRefs` readers already exist in the draw waiting for F9 to supply atom refs.
...
Measurement 6 RESULT 2026-08-30, operator screenshot after a full connector reconnect, verbatim strip: `script-ran handshake=ready caps=openLinks,downloadFile,serverTools,serverResources,logging,updateModelContext,sandbox message=text reply=none foreign=0 net=esri:ok200,gcs:ok404,svc7:ok200,self:opq gl=webgl2 bridge=ok`. Decode: Esri imagery tiles fetch with CORS (200); the GCS tile bucket is reachable with CORS (404 only because the probe path names no real object); services7 FeatureServer fetches (200); our own origin is reachable but opaque (health endpoint sends no CORS header; img tags would not care, fetch would need a header); WebGL2 is available; the app-initiated resources/read bridge WORKS. Consequence: option A (live trimmed map) passes BOTH gates, and transport B3 is also available for any server-rendered asset. One honest limit of the instrument: the cut declared the four origins in `_meta.ui.csp` AND fetches passed, so whether the declaration was necessary (host honors it) or the host simply does not restrict is not distinguishable from this run; no undeclared-origin control was probed. We keep declaring. Also learned operationally: the host caches the app resource per connector session at connect time and mounts it WITHOUT refetching across a URI bump (server log showed zero resources/read between 18:43 and the reconnect); only a full connector disconnect/reconnect picks up a new cut. That is now a standing walk-instruction fact. Earlier status: BUILT and canaried. p559 (`smartsite-mcp-00067-puw`, digest `sha256:c2b4c618...`) carries the three-token boot strip (`net= gl= bridge=`), declares the four origins in the resource's `_meta.ui.csp` (previously empty arrays), and is held at 0 percent; PR #556 squash-merged 28969a36; record `_inbox/2026-08-30_p91_p559_probe_deploy.md`. Shift on the operator's word, one reconnect, one screenshot of the strip; result lands here verbatim. Weighing ruled in chat 2026-08-30: E ships unconditionally as the default ground inside the v3 build; A is gated on this probe (declared-CSP fetch AND WebGL both passing) and sits on top of E if it passes; the snapshot underlay is the fallback via whichever transport the probe shows (`bridge=ok` preferred). A map click pushes to Claude through the same drafted Open turn as doors and board rows; paint-only prefetch over `serverTools` is a named fork for the UI conversation, not built.
...
3. Then the v3 WDLL, the gap ledger as its spine, rows opened on OPS-16. DRAFTED 2026-08-30 as a skeleton on the operator's go: `_inbox/2026-08-30_smartsite_mcp_app_v3_WDLL.md` (thirteen-function ledger on the ruled axis, S/M/Q/P build items, ruled exclusions, open measurements X1 to X4 and rulings R-1 to R-3, W2 function-scored walk frame). Refine by amendment; rows open on ratification. APPROVED by the operator 2026-08-30 ("skeleton approved"); the OPS-16 amendment row is queued behind the commit freeze because OPS-16 carries another seat's uncommitted edits.
