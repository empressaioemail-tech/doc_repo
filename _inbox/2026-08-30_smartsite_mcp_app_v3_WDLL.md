...
date: 2026-08-30
status: APPROVED (operator 2026-08-30 "skeleton approved"; refine by amendment; build NOT yet opened, rows open on OPS-16 once the commit freeze lifts, since OPS-16 currently carries another seat's uncommitted edits and an amendment row would entangle with them). FREEZE PREMISE DISSOLVED 2026-08-31: `git status` on `90_operations/OPS-16_texas_market_plan_of_record.md` is clean and its last commit is `7dfc69e`, so the entanglement this status conditioned on no longer exists. The v3 amendment row is unblocked and still unwritten; the last OPS-16 row is A-052. The freeze was not lifted by a ruling, its reason went away, which is a different thing and is why this is recorded rather than deleted.
...
| 7 | Area: around X, subdivision, street | STALE ROW, corrected 2026-08-31: the VIEW is built and serving (M1 anchor, M2 batch anchors, M3 map ground, M4 canvas, M5 territory honesty, all on p560 to p562). The FRONT DOOR is not built | selectors only: you can see an area only if you already name its parcels, and cannot ask for one | build Q1 to Q3 | Q1, Q2, Q3 |
...
M3. BUILT AND SERVING on p560, and this paragraph's design is SUPERSEDED, corrected 2026-08-31. MapLibre was rejected at CP1: it adds roughly 800 KB to a 76 KB resource and brings a WebGL dependency into a panel that renders without one. What shipped instead computes Web Mercator tile coordinates from the anchor in about a hundred lines and places an Esri tile mosaic as an UNDERLAY with the existing SVG drawn over it unchanged, so no map library is loaded and pan and zoom are not in this cut. Read `_inbox/2026-08-30_p91_v3_map_build_cp.md` and `_inbox/2026-08-30_p91_p560_deploy.md`, not the original text below. ORIGINAL SCOPE, retained for the record: the map ground, per measurement 6 (all gates passed): the trimmed live map. MapLibre inlined, aerial imagery and our tile origins fetched by URL under the declared CSP, WebGL2 confirmed, no PE chrome. E's vector layers (neighbor rings, road names, water, tints) ride on top and ship regardless as the no-imagery fallback rendering. A parcel tap resolves to a node and drafts the ordinary Open turn. `bridge=ok` is the transport for any server-rendered asset if ever needed. Esri terms check for this usage context is a named to-do before ship; NAIP-in-our-bucket remains the sovereign alternative (measurement of size and cost if chosen).
...
# 5b. Vocabulary and report shape (V items; added 2026-08-30 from the courier card)

Source `_inbox/2026-08-30_smartsite_report_styling_vocabulary_courier.md`, driven by an unscripted live session against three Bastrop parcels on the 2026-08-29 bake. Its central observation is architectural and it is correct: the display vocabulary lives ONLY in the panel, so any prose composed outside the panel drifts from the UI by construction. The model was never given the string the panel prints, so it wrote `atom_path_pending` into user-facing prose. That is not a model failure; it is a missing field.

The scoping insight that makes most of this cheap: the MCP server already normalizes every result (`sectionDisposition`, `declareUpstreamNonOk`, the `unread` pass-through), so it can attach display strings, per-facet guidance and an explicit derived-figures policy DURING normalization, with no cortex change and no dependency on another seat's files. That is better than putting it upstream, not merely cheaper: the panel and the model then read one table owned in one place, which is what makes drift impossible rather than unlikely.

V1. Vocabulary table, 15 to 20 entries, machine token to exact display string to a one-line meaning. Covers the disposition enum, the refusal codes, the two Open failure strings which must stay distinct sentences ("Open did not reach me" for a click with no tool result; "Not on file in Bastrop" for a miss), `citationsDegraded`, confidence `seed`, edge roles including `side_corner`, and frame quality `gis-approximate`. The four states that read alike, `unread`, `absent`, `absent-verified` and `unknown`, each get a display string a non-technical reader can tell apart, and `unknown` never renders as a clear finding.

V2. Publish it three ways: a canonical doc here, an MCP resource the assistant can read, and a compact standing block attached to every tool result, byte identical every time, lookup only and no behavioural instructions. Record in the doc what the courier card correctly notes: the block reaches the assistant only on turns where the connector is called, so it re-arms vocabulary and cannot hold framing across off-topic turns.

V3. `displayText` beside every machine code, attached in MCP normalization. V4. Facet-scoped `agentGuidance` extended from the one facet that carries it to every non-present facet; the card reports it as the highest-yield mechanism already in the payload and it is deployed once. V5. An explicit derived-figures deny: nothing today prohibits computing an area or a coverage ratio from `draw.ring`, and a session did exactly that. This is the same failure class as the tracked invented 42 percent lot coverage, so it becomes a stated policy in the payload rather than a convention. V6. Separate an overlay's `label` from its `finding` where `state` is `unknown`, so no consumer can render "No pipeline within 500 ft" as a result when nothing was checked.

V7. Report shape. Deterministic output belongs in `export_instrument` server side (BLOCKED: substrate seat endpoint). Guided composition belongs in a declared MCP prompt, working name `parcel_brief`, which versions with our deploy rather than with the assistant's judgment. A worked exemplar belongs in resources, and must include a refused facet and an `unknown` overlay so it demonstrates the hard cases. Free composition stays for what needs judgment: in the observed session the assistant's real value was cross-parcel reasoning, including the reciprocal edge-length discrepancy this program independently recorded the same day, and templating that away would be a loss. Candidate canonical section order: On record, Withheld, Not established (with `unknown` stated as unchecked), Verification pointers.

V8. Walk coverage: a vocabulary re-arming walk (establish, three unrelated turns, return, grade the display strings), a derived-figure trap where the geometry invites an area computation and any square footage or percentage not present as a field is a fail, an unknown-versus-clear trap on pipeline and specialDistrict, all graded on the painted string rather than the tool JSON.

Constraint carried verbatim from the card because it is the right frame: expect roughly 80 percent adherence with graceful decay from any prompt-based mechanism, so anything that must be TRUE rather than LIKELY belongs in the payload or in server-side rendering, never in guidance.

Cortex-side and therefore not in this thread: none of V1 to V6 requires cortex if attached during MCP normalization, which is the recommendation. If a later ruling moves any of them into the bake instead, they become another seat's work.

Separate investigation, flagged by the card and not folded into a build item: the drainage facet returned `unread` with no producer and no code stamped on all three parcels, across two zoning districts and three lot shapes. This program's own measurement 5 independently established that NO drainage producer exists anywhere in the bake, which explains the observation and makes it facet-wide rather than parcel-specific. Three parcels was not proof; the write-path read is.
