...
## COTALITY DECOMMISSION + NO-SETBACK EXPORT — both DONE + deployed + live-verified (2026-07-28)

TASK 1 — no-setback site-plan export: DEPLOYED CLEAN. Engine no longer 422s when a parcel lacks a setback-rule atom; full sheet exports with setbacks honest-absent ("not specified — no rule on file, not verified"), never fabricated. Live: engine-api `00107-yuc` @100% (no-setback + 4Gi OOM-fix + bbox-guard in one revision), MCP `00032-gzm` @100% (env preserved), PE prod. PRs engine #163, MCP #51, PE #90 (all CI-green, merged). Also fixed a latent ring-geometry bug (all-zero inset wrongly flagged offsetDegenerate).

TASK 2 — envelope 502 = COTALITY DECOMMISSION (NOT a credential fix): DONE + live-verified. Root cause: cortex-api `queryGisLayerGeoJson` fell through to the dead-keyed Cotality Spatial Tile when, in the default `county-gis` mode, a point resolved to no supported county (Cotality OAuth 502 "invalid client identifier"). Per standing decision (Cotality extinguished — re-route to public record, NEVER rotate the credential): in county-gis mode a no-county point now throws a named `no-coverage` AdapterRunError → the envelope handler maps it to an honest 404 "no parcel here" → Cotality NEVER called. Cotality retained ONLY behind the explicit `TX_PARCEL_PROVIDER=off` escape hatch (unset in prod). ldt PR #361 (`legacy-design-tools`, `artifacts/api-server/src/lib/brokerageGisLayers.ts`) merged. CI caught a test that codified the old dead-vendor fall-through (`"out-of-coverage bbox flows to the Cotality branch unchanged"`) — updated to assert the honest no-coverage decline (spy fails if Cotality called) + a companion off-mode escape-hatch test; merge-only-on-green held.
- DEPLOYED via the cortex-api canary form (push-build → workflow_dispatch deploy-canary → smoke → shift-traffic). Prod now serves `cortex-api-00442-heq` @100%.
- LIVE-VERIFIED on prod (Node fetch, curl egress-blocked by local AV proxy): Bastrop `1001 Chestnut area` (30.1105,-97.3155) → **HTTP 404 no-setbacks, node 48021:34785, is502=false** (county-GIS resolved the real parcel; honest "no setback table yet" decline). Bastrop gold coord (30.11,-97.32) → **404 no-parcel, is502=false** (honest county-GIS "no parcel at point"). ZERO Cotality OAuth calls in canary logs. The old 502 "Parcel geometry provider unavailable" is GONE.

PLANNER MISS (owned): initially framed the envelope 502 as "fix the Cotality credential" — wrong; Cotality was extinguished 07-13 (06-17 "sole spine" reversed), so hitting it is a routing/decommission bug. The standing-decisions block corrected it. Lesson reinforced: check a vendor's decommission status against program history before proposing a credential fix.
