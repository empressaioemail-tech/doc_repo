...
| wave 3 | P-173 | LEASE HISTORY | An append-only `atoms_writer_lease_history` written by take and release in `packages/storage/src/atoms-writer-lease.ts`; the P-171 audit script reads it. | a staging writer run leaves a history row that survives release; the audit script returns it by run id | ADDED 2026-09-12 (A-133) |
| wave 3 | P-174 | ONE PLACEMENT PATH | Search, id, shared link and click seal one sheet through one function; the path or cache that seals without setbacks deleted. | a search for `1109 PECAN ST` shows setbacks with no second click (operator observed) | ADDED 2026-09-12 (A-134, F20), wave-3 addendum |
...
| F21 the record-served PE facets payload carries no `cityLimitsFact.queryPoint` (every parcel, 2026-09-13); the probe's near-bbox and ring legs are unmeasurable on the record path and a search landing has no record point to seed placement from (candidate P-174 mechanism) | P-152 lane 3 (restore the point), P-174 (test it) |
