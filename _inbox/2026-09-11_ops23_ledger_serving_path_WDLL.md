...
| 3 | P-153 | DRAW | Amber inset visible on the map for `48021:34049` for an entitled viewer; MCP draw block carries the same polygon with the same vertex count; no surface prints a figure; `48453:474034` still refuses. | `surface-probe --rows P-153` with observations | IN FLIGHT at 2026-09-12 02:40Z: hauska-map #384 `1aef5dd5` merged 22:02Z but inert on the surface (the lane's own live probe found it), #386 `8b44f68d` merged 22:33Z wires the augmentation; LDT half MERGED: #661 `6b579020` at 2026-09-12T00:00:04Z, SUCCESS (the planner's and the overseer's `gh pr list --search p153` both missed it; the title reads "P-153/MCP"; read PRs by number). Both halves merged; hauska-map deployed by the lane; cortex-api `00778-peq` was built from origin/main after #661 and carries it; smartsite-mcp's serving revision checked by field on 2026-09-12 (see the session record). CLOSED 2026-09-12 by the overseer from the lane's two checkpoints: the operator's screenshot shows the amber inset drawn on `48021:34049` with the Buildable row "modelled from setbacks, area withheld pending an atom" and no figure; the lane's MCP observation carries six vertices, figure not printed; `48453:474034` refused with no polygon (operator screenshot 2026-09-11). Probe PASS on both parcels (artifact named in `_inbox/2026-09-12_p153-draw_close.json`). Tier that sees the drawing: Studio or Team (the owner row rendered). P-167 unblocked |
| 4 | P-152 | READER (step 5) | retrieval-api holds a read-only factory role, walks gated cells, dereferences atoms, serves the cached rendering on key match, refuses unslated rails; cortex, the panel, smartsite-mcp and the engine reports consume it; the 70 legacy paths from L1's audit answer 404 or a declared decline; panel `bakedAt` is the reader's snapshot. | `surface-probe --rows P-152` (panel versus MCP disagreement count zero); retired routes probed for 404 | lane 1 of 2 (P152-READER) dispatched 2026-09-11; per its CP2 (21:05Z): engine half merged (hauska-engine #417 `6f1b7ffe`, #418 `419ae8c8`, #419), RO secret mounted on operator approval, canary `hauska-retrieval-api-00086-nur` at 0 percent answers `/record` 200 for all five probe parcels (34049: 65 rails, 21 `record`, 44 `legacy-transitional`, cityLimits byte-identical to a direct read); a third direct factory connection found and repointed (`parcelRecordFactRead.ts`, flood into the brief); LDT #658 `55c4ad44` merged 21:42Z; by field read 2026-09-12 02:40Z `hauska-retrieval-api-00086-nur` and `cortex-api-00776-wov` serve 100 percent; five-parcel facets diff byte-identical on every slated rail; lane 1 CLOSED (`_inbox/2026-09-11_p152-reader_close.json`, partial at row level: the row predicate is UNMEASURED until P152-PANEL; surgical swap kept the 18 rail-pair files as thin wrappers on operator approval; `FACTORY_DATABASE_URL_RO` out of the cortex deploy); lane 2 (P152-PANEL) CLOSED-PARTIAL 2026-09-12: the panel reads the reader (hauska-map #387 to #390, all SUCCESS; live `X-Pe-Read-Path: record` on all five parcels; 14 composer rails, 9 to 11 `record` per parcel, the acreage trio and agValuation and maxImpervious `legacy-transitional`; the studio-gated collapse fixed); the row predicate FAILS on `48021:34049` and `48021:33223` because the setback and zoning rails still ride the atom chain (F3) and the MCP reads the record; NEXT: lane 3 composes setbacks and zoning from the reader and makes the report composer the third consumer with the requester's entitlement (F17, F18, F19). WAVE 3 (2026-09-13, overseer): lane 3 CLOSED-PARTIAL; row stays PARTIAL. Report composer reads the reader (hauska-engine #426, #431, #433; `hauska-engine-api-00211-vam` at 100 percent with `RETRIEVAL_API_URL` and the RO key mounted); panel `setbackFrontFt` and `zoningDistrict` serve record (hauska-map #393), five rails legacy-transitional; F21 fixed (#396, the query point is back on the record path). Instrument 2026-09-13: panel and MCP setbacks still DISAGREE on 34049 (30/5/25/15 vs 30/10/30/20) and 33223; the wave close's "closed PASS" for this row is NOT accepted (`_inbox/2026-09-12_p152-rails_close.json`, annotated). Remaining: the five rails, item 2 (dollar rails and owner gated in the BFF), the engine-side entitlement gate. WAVE 4: lane 4 `p152-slate` slates the ten legacy-transitional rails after the Bastrop re-run and gates the panel's dollars and owner; lane 5 `p152-entitlement` puts entitlement in the engine and the MCP. WAVE 4 (2026-09-13): lane 5 CLOSED (engine gates dollars and owner on the tier, #438; MCP passes the real tier, #673; verified both directions); lane 4 CLOSED-PARTIAL (slate lives in engine `parcel-record-slate.json` + LDT `parcelRecordAllowlist.ts`; nine pairs pass now; five siblings need per-rail entries, RULED A-140; panel axis override in hauska-map is the remaining bug). Row PARTIAL. WAVE 5: lane 6 `p152-siblings` (per-rail slate entries, nine safe pairs, panel record-first) |
| 5 | P-154 | MOST-CURRENT (first family through step 3) | One resolver, date-first; engine adapter and factory writer use it; Bastrop ledger setback cells re-run; the three Bastrop dates recorded. | four surfaces identical for `48021:34049` or all four show the conflict row | not dispatched. WAVE 3 (2026-09-13): CLOSED-PARTIAL. Most-current resolver built and live in hauska-engine (#424 after a forced rebase; factory #141); legacy-design-tools still ranks tier-first and the panel prints layer 23's values; instrument reads panel and endpoint DISAGREE on both Bastrop parcels. A sixth live setback producer (`bastrop-per-parcel-setback.ts:75`) has no test. Sharing the resolver needs a substrate worktree grant or an adapters package publish, an operator ruling (`_inbox/2026-09-12_p154-most-current_close.json`). RULED 2026-09-13 (A-138): the resolver is SHARED through `@empressaio/setback-corpus` `./resolve` 1.2.0; wave-4 lane `p154-share` (four worktrees) executes it and re-runs the Bastrop cells. WAVE 4 (2026-09-13): CLOSED-PARTIAL, the ruling executed (corpus 1.2.0 `./resolve`; engine #437, LDT #674 consume it; MCP and endpoint 30/10/30/20 dated 2026-04-14); the panel still prints 30/5/25/15 from hauska-map's axis override; the factory router never had a local rule (mission premise false); Bastrop re-run changed 0 cells (write-once gate) |
| 6 | P-155 | FEASIBILITY | Refresh returns 202 with a job reference; clients poll download; the cold-start string is gone; Travis versus Bastrop latency profiled and recorded, not gated on. | `export_instrument feasibility 48453:474034` returns a PDF; the app produces it without a retry | CLOSED-PARTIAL 2026-09-11 (engine #420 `81ec7b09`, hauska-map #385 `91990741`, LDT poll fix; `hauska-engine-api-00205-san` at 100 percent carries it with P-159): refresh 202 and MCP PDF OBSERVED on both parcels; CLOSED 2026-09-12: the app produced FS-48453-474034 on the first click (16:56Z, 13 pages; operator: "no, the first time I clicked it ran"); the probe's P-155 row aligned to the card's parcel and reads PASS; leave-behind: no traffic-shift lease (P-155 and P-159 collided live), `ENGINE_API_GATE_TOKEN` unset in production, 10 min stall ceiling near the slowest observed job (630 s), CPU throttled between requests. REOPENED 2026-09-13 (A-143, F23): the refresh hop no longer produces a new document; wave-5 addendum lane `p155-refresh` |
...
| 8 | P-157 | STRUCTURAL | TCAD improvement detail ingested; living area and year built present for both Travis probe parcels; county null rate on improved parcels reported before and after. | `structuralFact` on the probe set | BLOCKED at CP1 2026-09-11 (`_inbox/2026-09-11_p157-structural_cp1.json`): no cloud execution path exists for LDT `lib/cad-ingest` (36 Cloud Run jobs, all factory); the TCAD certified export names its files `PROP.TXT` and `IMP_DET.TXT`, not the loader's `*_APPRAISAL_INFO.TXT` pattern; three operator questions: break-glass or infra card, who owns the PACS file-name registry, which roll vintage (preliminary 07072026 or certified 07182026). RULED 2026-09-12: no break-glass; resumes after P-169; certified 07182026 for both files; the file-name shape is a source declaration in the loader registry. WAVE 2: the load ran through `ldt-cad-ingest` on staging and production (real files, hashes, `cad_ingest_run` rows; the job needed 16Gi/4cpu) and the improved-parcel null count did not move (397,065 to 396,743 improved, 100 percent null both times): the shared PACS parser rolls living area up from segments typed "MAIN AREA" and TCAD types them "1st Floor / 2nd Floor / 3rd Floor" (typeCd 1ST/2ND/3RD). BLOCKED on the parser: a per-county segment vocabulary in the loader registry, then the fill. RULED 2026-09-12: the P-157 lane owns the parser fix (wave 3). WAVE 3 (2026-09-13): CLOSED PASS. The parser reads TCAD's segment vocabulary as a per-county declaration (LDT #666); re-loaded on staging and production; both Travis probe parcels present (3,550 sq ft 1950; 4,168 sq ft 2001), reproduced by the overseer; production read: improved accounts 396,743, living area null on 2,702 (0.68 percent) (`_inbox/2026-09-13_p157-structural-r2_close.json`; the wave-2 blocked close kept) |
| 9 | P-158 | FOOTPRINT | The 10 percent join's denominator stated; the predicate fixed if it is parcel area; the map layer reads staged footprints, not the Bastrop-only atom table. | `buildingFootprintFact` present for `48021:34049` and `48453:113408`; near-bbox returns at least one footprint around each | CLOSED-PARTIAL 2026-09-11: F8 was stale when written (atoms for 48021 and 48453 were minted 2026-09-07: 24,861 present / 46,640 absent and 72,919 / 323,543); engine PR #421 (label split into three evidenced causes, CI SUCCESS) OPEN for the overseer; the anchor parcel's nearest staged footprint is 1,615 m away, which the lane calls a source coverage gap and the overseer reads as a staged-layer load gap until a direct count of `tx_building_footprint` inside Bastrop city limits says otherwise; no sanctioned execution path for the footprint writer; unexplained 2026-09-07 production write with no execution log (rowed as P-171). RULED 2026-09-12: merge #421; count before any source. WAVE 2 (#421 merged `fcd77075`): the count decided it and neither reading was right: `tx_building_footprint.county_fips` is MISTAGGED near county lines. Inside Bastrop city limits 3,987 footprints exist and 1,182 (29.6 percent) carry 48021; West Lake Hills' footprints inside Travis are all tagged Hays 48209; 80 footprints lie within 200 m of `48021:34049`, the nearest at 1.63 m, none under the writer's own county filter. No second source. PHASE 3: retag by true containment against `tx_county_boundary` (the fix sits in the unmerged loader branch `feat/p2-4-tx-building-footprint-staging`), then the writer through `hauska-engine-atoms-writer` and the factory reconcile for 48021 and 48453. WAVE 3 (2026-09-13): CLOSED PASS. Phase 3 retag by containment; three real defects fixed on the way (hauska-engine #434 OOM, #435 a DISTINCT ON key that silently dropped 15.2 percent of Bastrop rows, #436 a composite GiST index); near-bbox count 42 (34049) and 15 (113408) reproduced by the overseer on two runs after three transient 504s on the Travis leg (`_inbox/2026-09-13_p158-footprint-phase3_close.json`; the phase-2 close kept). Leave-behind: the reconcile job has no staging target |
...
| step 1 | P-161 | NODES | The three OPS-22 §1 identity rulings taken; `placeKeyToEntityId` published in the atom-contract package with self-tests; the `27303` versus `027303` collision population measured; one key per city, district and road. | the function is imported by factory and engine; the collision count is in a file | not dispatched. RULING 2026-09-13 (A-139): the node id is the parcel-map id; the account is an attribute through the crosswalk; P-161 carries the crosswalk type (`parcel-account-crosswalk`: both identifiers plus the corroboration) |
...
| vocab | P-167 | VOCABULARY | One display module as a subpath of the atom-contract package, imported by writer, reader, PE and engine; the parity lock deleted. | both repos import the package; display strings identical across surfaces for the probe set | CLOSED-PARTIAL 2026-09-11: step 1 done, `@empressaio/atom-contract@1.32.0` live with `./display` (PR #28 `995df513`, three CI legs SUCCESS); steps 2 to 5 (fold P-153's token as 1.33.0, consumer imports, copy retirement, the surface probe) wait on P-153 merging in both repos. WAVE 3 (2026-09-13): CLOSED-PARTIAL. `@empressaio/atom-contract` 1.33.1 on npm; hauska-map and hauska-engine declare ^1.33.1; legacy-design-tools still declares ^1.30.0 (overseer read of origin/main); no vocab parity-lock file survives; the PDF leg unverified; the instrument reads UNMEASURED (no `_vocab` observation it can consume) (`_inbox/2026-09-12_p167-vocab_close.json`). WAVE 4: lane `p167-pdf` closes the LDT pin, the PDF leg on a revision containing #429, and writes the `_vocab` observation the instrument reads. WAVE 4 (2026-09-13): CLOSED-PARTIAL; LDT pinned ^1.33.1 in all seven files (#672), MCP and cortex redeployed; strings still differ for three named reasons (`no-zoning-stamp` token missing; `site-model.ts:658-660` hardcoded PDF labels; zoningProvenance has no panel wire field). WAVE 5: lane `p167-strings` (the `no-zoning-stamp` token, `site-model.ts` labels from the vocabulary, the zoningProvenance wire field). WAVE 5 lane closed-partial 2026-09-13: token 1.34.0 live, engine and hauska-map cut over and deployed; LDT lockfile still 1.33.1 (overseer's mission omission; addendum), zoningProvenance invisible until lane 6 slates the rail, PDF leg blocked by F23 |
...
| wave 2 | P-169 | INFRA | Cloud Run jobs for LDT `lib/cad-ingest` and engine `write-building-footprint-county.mjs`, factory job pattern, load manifest, code-level laptop refusal; TCAD file names as a source declaration. | `gcloud run jobs list` shows both; a staging dry run leaves its record; a laptop `--apply` is refused by code | CLOSED 2026-09-12 PASS: `ldt-cad-ingest` (LDT #665 `08de93dc`) and `hauska-engine-atoms-writer` (engine #423 `99f9146f`) in `hauska-prod-497015` us-east4, images by digest in the close; staging runs left a `cad_ingest_run` row and a lease row; laptop `--apply` refused with a structured `LAPTOP_WRITE_FROZEN` payload. Contradicted: `Dockerfile.atoms-writer` lives in hauska-engine and the atoms-writer job pattern already existed there; the real gap was the target selector and the code-level refusal for the footprint writer. Leave-behind: fold P-157's 16Gi/4cpu bump into the job spec |
| wave 2 | P-170 | TRAFFIC LEASE | One traffic shift at a time per service: contract clause, `_catalog/leases/`, hook `traffic-lease-gate`. | the hook observed refusing a shift without a lease, through the harness | BUILT 2026-09-12 03:10Z (`scripts/enforcement/traffic-lease-gate.mjs`, hook wrapper, registered on Bash, `_catalog/leases/` ignored by git): self-test 17 checks both directions; direct invocation refuses without a lease and allows with one; harness firing owed the next session. The wave-2 planner used the lease for the cortex-api shift (granted, shifted to `00778-peq`, released after the probe) |
| wave 2 | P-171 | PROVENANCE | The 2026-09-07 footprint atoms write named: writer, run record or break-glass row after the fact, lease rows for the scope. | the close carries the record or the named writer | CLOSED 2026-09-12 PASS, outcome (b): the writer was the hauska-engine lane session `cente-67` running `--apply` from a P:/ worktree overnight 2026-09-06/07 for five counties after engine #391 and #392, under the operator's contemporaneous sequencing direction, with no break-glass row filed; the row `_decisions/2026-09-07_footprint_writer_breakglass_five_counties.md` was ACCEPTED AS WRITTEN by the operator 2026-09-12 (active). Logging gap rowed as P-173 |
| wave 2 | P-172 | FIND BOX | Address form resolves from the situs index; parcel id resolves directly; geocoder only as a labelled fallback. | Find box resolves `414 SPILLER LN` to `48453:113408` (observed); the search route returns the id (machine) | CLOSED 2026-09-12 PASS (hauska-map #391 `4350ab95`, LDT #664 `78ad8a51`; cortex-api `00778-peq` at 100 percent): the index row backfilled from the roll, both query forms resolve via the situs index in under 50 ms, the geocoder rung labelled. Contradicted: the 5 s timeout was a proxy budget of 5 s against a route budget of 20 s, not a slow scan. The operator's own click on the live Find box landed on the card first try (2026-09-12); row CLOSED on both legs |
| wave 3 | P-173 | LEASE HISTORY | An append-only `atoms_writer_lease_history` written by take and release in `packages/storage/src/atoms-writer-lease.ts`; the P-171 audit script reads it. | a staging writer run leaves a history row that survives release; the audit script returns it by run id | ADDED 2026-09-12 (A-133). WAVE 3 (2026-09-13): CLOSED on the planner's artifact (hauska-engine #425; append-only `atoms_writer_lease_history` with a trigger; DELETE, UPDATE and TRUNCATE refused live; a real take/release on staging and production; `_inbox/2026-09-12_184520_surface_probe.json` PASS). Not reproduced by the overseer (engine store credential) |
| wave 3 | P-174 | ONE PLACEMENT PATH | Search, id, shared link and click seal one sheet through one function; the path or cache that seals without setbacks deleted. | a search for `1109 PECAN ST` shows setbacks with no second click (operator observed) | ADDED 2026-09-12 (A-134, F20), wave-3 addendum. WAVE 3 (2026-09-13): CLOSED on the operator's controlled re-test (13:00Z, More Facts expanded both times) after hauska-map #394 (cache evicts on a pending seal), #396 (F21) and #397 (bounded situs-search timeout); reproduced by the overseer with the same observation. The operator's 00:05Z failing observation is kept (`_inbox/2026-09-12_p174_observations.json`) |
...
| F20 a search-landed subject seals without setbacks until the user clicks away and back; two placement paths, one parcel (operator 2026-09-12) | P-174 |
| F21 the record-served PE facets payload carries no `cityLimitsFact.queryPoint` (every parcel, 2026-09-13); the probe's near-bbox and ring legs are unmeasurable on the record path and a search landing has no record point to seed placement from (candidate P-174 mechanism) | P-152 lane 3 (restore the point), P-174 (test it). FIXED 2026-09-13 by hauska-map #396: `applyRecordPatch` replaced `cityLimits` with the reader's cell, which has no point; the point is back and P-158's leg measures again |
| F22 `hauska-engine-api` is public (`allUsers` invoker, ingress all) and its gate token does not exist in either project; the bearer check is a no-op (wave-4 Q5, confirmed by the overseer 2026-09-13) | P-179 |
| F23 the feasibility refresh produces no new document (two exports a minute apart byte-identical, created 19:12Z, before the 21:30Z engine deploy; overseer 2026-09-13) | P-155 reopened, wave-5 addendum `p155-refresh` |
