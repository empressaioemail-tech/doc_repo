...
## LOCKED 2026-07-29 — planner code-review amendments (v2; these OVERRIDE any conflicting text below)

Ratified by the operator after a full code-verification pass. Build agents follow THESE where they differ from the body:

1. 3-FREE-MESSAGES RULING (operator-ratified): the 3 free AI messages per property are **SIGNED-IN-FREE** — 3 messages per free ACCOUNT per property, SERVER-counted (small cortex table keyed user+parcel, checked in the chat route BEFORE the paid gate). The free inspect card + map browse stay fully anonymous; the chat taste requires a free sign-in. Truly-anonymous metering is REJECTED for v1 (needs an anonymous-tolerant chat route + anonymous identity = the auth-orphan trap + an LLM-cost abuse surface). The pricing spec's "no login needed" applies to the card/map, not the chat messages.
2. R2 SCOPE CORRECTION: the atom-fetch endpoint ALREADY EXISTS — `GET atoms/:did` on retrieval is reachable through PE's spine proxy (spine.ts:170; anonymous-allowlisted). R2's new work is: (a) chip fetch-on-tap wiring, (b) the BRIEF/FULL accordion, (c) LINEAGE v1 CLIENT-COMPOSED from the property's atom-chain (envelope←setback←zoning←code links are structurally present in the existing atom-chain call; a true lineage/graph route is a later upgrade, NOT v1), (d) VERIFY gated-404 uniformity on atoms/:did (forbidden == missing; do not assume — test it), (e) PRESENTATION MODE: the chat currently sends presentationMode "consumer", and consumer mode STRIPS [n] markers from answer text (brokerageBriefLlm: "Do NOT include [n] citation markers") — switch the workbench chat to the PROFESSIONAL mode so inline citations render for the pro ICP. Backend anti-fabrication constraint CONFIRMED existing ("Use ONLY the numbered code atom sources. Do not invent code." + parseInlineCitations drops out-of-range [n]) — R2 verifies + hardens with tests, does not rebuild. The citations spec's streaming-safe rule is MOOT v1 (chat is single-shot JSON; no streaming path).
3. R1 ADDITIONS: the property-scoped entitlement check must land SERVER-SIDE on every paid route (requirePePaidDeep brief/chat paths, fetchPeEntitlement in the export BFFs, the share mint) — a client-only gate is cosmetic; this is a cortex schema+middleware change + cortex deploy (canary workflow). Edge rules: TERRAIN excluded from the per-property unlock (Pro-only); SHARE mint folds into per-property semantics (share is a paid bubble); the WB6 chat-summary call is classified as part of PAID chat (not a free message). SPLIT R1: the chat free-counter (per ruling #1) is its own slice so nothing else blocks on it. Replace the current Pro-hardcoded paywall copy (PaywallGate/ExplorerMap "R1–R10 … Pro entitlement") with the unified two-choice unlock flow. Reserved-atom-color audit rides R2 (one accent = openable recorded evidence, reused nowhere).
4. R3 ADDITIONS: (i) the parcel-scoped drainage study needs a NEW ENGINE ROUTE (CC's is cortex-side; engine /drainage expects a client DEM) — adopt the queued H3 scoping; (ii) NOAA Atlas-14 forcing input lives in ldt's ingest path — port engine-side or parameterize the depth (state the choice); (iii) the report PDF builds on WB7b's dossier-document scaffolding (merge WB7b FIRST — one document pattern, never two); (iv) VERCEL FUNCTION BUDGET: PE is at 11/12 Hobby-cap functions — the report BFF must fold into an existing function (e.g. a reports dispatcher), NOT add a 12th+ (the #108 deploy failed exactly this way; see #109).
5. INVENTORY TIME-DRIFT: hydrography is FULLY live end-to-end (engine #175 deployed + verified, PE #108, consolidation #109); the backend re-warm is PARTIAL (envelope re-promote done+verified; facet re-bake in flight; W16 situs restamp queued) — do not gate R1/R2 on it; R3's live-verify must run against post-restamp data. WB7a (cortex share route) + WB7b (engine dossier PDF) are at CI on the planner's board.
