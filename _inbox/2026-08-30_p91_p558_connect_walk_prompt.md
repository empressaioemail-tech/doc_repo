...
# Run record (partial, 2026-08-30, after the approval gate cleared)

The two frontmatter run notes above record the GATE STOPS ("No approval received", the call never reaching the server). This section records the run that got through, so the two readings do not contradict: the earlier stops were host consent, the later run is the measurement.

Gate: catalog 13, names matching the standing set. URI not confirmable from the agent's side (no resource URI in the transcript); confirmed instead by the planner server-side via `resources/read`, which returned `ui://smartsite/app-p558.html` at 75.7 KB while a retired URI returned not-found. Boot strip captured by the operator: `script-ran handshake=ready caps=openLinks,downloadFile,serverTools,serverResources,logging,updateModelContext,sandbox message=text reply=none foreign=0`.

Step 1 PASS on both halves. Screen id `3ae57618-6dba-4221-8c1e-bbf22559e79f`, six rows written (the Cove spelling is not a row), `degraded.duplicates` naming `111 Rainmaker Cove, Bastrop TX 78602` with `keptQuery` `111 Rainmaker Cv, Bastrop TX`, zzzz row `parcelNodeId: null` with no stub block. Operator paint: unresolved row first with all-gold rails and a Look this up control and no Open, rail glyphs on every resolved row, the not-added-twice note under the board, `Rows by completeness` sort control, legend present. No county headers because every row is Bastrop, which is the rule not a miss.

Step 2 measured, with a deviation that changed nothing: the script named 108 Higgins (`48021:31272`); the operator clicked Open on 102 Higgins (`48021:31254`) instead. Both ids are correct for their addresses per the fixture set; the deviation is which parcel was walked, not a conflict in the record. The Open path passed: the text the agent received is the app's own `OPEN_TURN_OPENER` draft verbatim, and the board line read `Sent to chat. Press Send to open.` Panel paint confirmed by screenshot: ring, `Zone X` above it, `SF-1` underlined with `bastrop_city_tx` beneath, north arrow, `50 ft unit reference`, the point-at-a-line hint, five edges with role, adjacency, ids, length and bearing, `why` controls on every non-present row, the envelope reason printed as `Withheld, setbacks unruled` (never the machine string), the flood row as facts with source, vintage and evaluated-at. Steps 3 through 8 remain unmeasured.

Finding from step 2, owner data lane, not a panel defect: on `48021:31254` four of five edges claim `adjacency: ROW`, naming OSM ways 1409946229, 1189268271, 30695153 and 15109717. Read live from the OSM API those are, in order, `highway=footway` (concrete), `highway=service` unpaved (an alley), `highway=residential` Perkins Street, and `highway=residential` Higgins Street. A sidewalk classified as a right of way is an edge-writer classifier defect, and it matters because ROW suppresses the neighbor door (O3). The two edges carrying both ROW and a neighbor id are not ambiguous in the panel: ROW wins and prints `across the right of way` with no Open. Whether a parcel across an alley should be a door is a ruling, not a bug.

# Finding, 2026-08-30 walk on p560: one shared boundary reports two different lengths

Owner data lane. Not a panel defect and not an app defect: the panel prints what the edge object carries (I7), and it carried both numbers faithfully.

The side shared by 906 Pine (`48021:34169`) and 905 Pecan (`48021:34161`) is 113.28 ft when read from 34169 and 138.36 ft when read from 34161. A shared line has one length. Three reads reproduce it: two of 34169 (byte identical, same runId, same bake) and one of 34161, so it is stable in the 2026-08-29 bake rather than a transient or a flaky response.

Ruled out by the walk before reporting: a wrong neighbor id. The two parcels' anchors, now on the wire since p560, sit a distance apart consistent with the lots touching, so the pair is correctly identified and the disagreement is in the measurement on one side of it.

Why it matters beyond one block. This is the shape of defect that the honest-absence discipline cannot catch, because both numbers are `state: "present"` and both look reasonable alone. It is only visible when two parcels are read together, which is precisely what a multi-parcel view makes routine. Two independently derived values of one quantity that disagree is the meaning-shaped check the enforcement doctrine asks for, and the edge writer currently has no such check: nothing compares an edge against the same edge as seen from the neighbor.

Recommended, not built: a reconciliation check in the edge writer that fails when a shared boundary's two derivations disagree beyond a stated tolerance. Until then, treat any single edge length as one party's claim.
