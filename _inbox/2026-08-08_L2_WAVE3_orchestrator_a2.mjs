...
const WORKER = join(INBOX, "2026-08-08_L2_WAVE3_worker_a2.mjs");
const MEMBERSHIP = process.env.WAVE3_MEMBERSHIP
  ? process.env.WAVE3_MEMBERSHIP
  : join(INBOX, "2026-08-08_L2_WAVE3_membership.json");

// Donley 48129 — 404 at source, never a wave member.
// Wood 48499 — RULED PARK 2026-08-08 (single corrupt source feature at lat 13.92,
// Prop_ID=0 placeholder). Named absence, not a silent drop. Guard untouched.
// See _inbox/2026-08-08_L2_WAVE3_WOOD_48499_RULING.md
const FORBIDDEN = new Set(["48129", "48499"]);
...
function writeProgress(state) {
  writeFileSync(
    join(INBOX, `2026-08-08_L2_${WAVE_TAG}_progress_a2.json`),
...
async function main() {
  ...
  writeFileSync(
    join(INBOX, `2026-08-08_L2_${WAVE_TAG}_results_a2.json`),
  ...
  writeFileSync(
    join(INBOX, `2026-08-08_L2_${WAVE_TAG}_report_a2.md`),
    lines.join("\n"),
    "utf8"
  );
    ...
    JSON.stringify(
      {
        ...
        totalRows,
        finalDistinct,
        results: join(INBOX, `2026-08-08_L2_${WAVE_TAG}_results_a2.json`),
        report: join(INBOX, `2026-08-08_L2_${WAVE_TAG}_report_a2.md`),
      },
      null,
      2
    )
