...
|-------|------|--------|-------|
| P2-shell | legacy-design-tools / cortex-api | COMPLETE — merged (PR #221, c84af22); cortex-api-00281-joy @100% | prominent HeaderSearchBar (typeahead); edit/view .ts-seamless fuse + mount-once portal + FloatingTileLayer dock-back + template reflow; grid|list layoutMode; server-persisted shareable spaces (saved_workspace_spaces, migration 0049, keyed tenant_id+owner_user_id -> tenant-private via WHERE-tightening); Module Map (persona-mapped, live registry). Reviewer PASS, 3 bugs fixed. Also survived a tmp-recycle via the insurance checkpoint (no loss). |
| P2-spine | hauska-map (console 5174) | COMPLETE — merged (PR #4, 678517d) | Control Tower skeleton (ControlCenterLayout + PanelRegistry + hash routing + primitives) in apps/command-center; 3 panels live (Atom Inspector via MCP search_atoms with never-bare {value,n,width,basis,scope}; Run Monitor; Surface&Gate), Calibration honest-empty, 5 stubbed; E1-E7 console unregressed. RECOVERED from a tmp-clone recycle (rebuilt + pushed-early); operator clone P:\hauska-map restored to clean main. |
| file->atom (engine) | hauska-engine / engine-api | COMPLETE — merged (PR #78, 52f760c); hauska-engine-api-00026-dop @100% | POST /v1/document-ingest live; point-to default (sourceDocumentCid); firewall VERIFIED live (private caller requesting public-paid clamped to tenant-private on blob+atom); confidence kind:'asserted'; idempotent; never-500 honest degrade; reviewer PASS 6/6. Follow-ups: (1) persistent DocumentIngestStore (Postgres+GCS) — in-process today, not durable across restarts -> dispatched; (2) ENGINE_API_GATE_TOKEN unset (gate bearer dev-disabled) -> Phase-3 gate work. |
| file->atom durability | hauska-engine / engine-api | COMPLETE — merged (PR #79, 7e15710); hauska-engine-api-00029-buy @100% | persistent DocumentIngestStore: Postgres (document_blobs + document_ingest_atoms, migration 004) + GCS (new tenant-private bucket). Restart-durable idempotency VERIFIED live (re-mint reads Postgres, created:false). health documentIngestStore:"durable". FINDING: engine had NO pg atom persistence to reuse (code atoms are in-memory/snapshot) + NO GCS wiring — this is the first durably-persisted atom family in the spine. |
| Dataroom tile | legacy-design-tools / cortex-api | COMPLETE — merged (PR #222, c6ba01f); cortex-api-00284-zuq @100% | Dataroom/Files tile: upload (Track D presign) -> per-file "Extract atoms" -> BFF ingest proxy -> engine /v1/document-ingest -> atoms render as cited confidence-graded chips linked to sourceDocumentCid; firewall confirmed (proxy sends no accessPolicy + tenant-private tier); migration 0050 (dataroom_document_atoms). Reviewer PASS. |

## PHASE 2 COMPLETE — 2026-07-02

All Phase-2 tracks merged + live: P2-spine (admin console), P2-shell (header search + fuse layouts + spaces + Module Map), engine file->atom + durability, Dataroom tile. Gated residual: map overlays draw (publish 0.1.1 needs NPM_TOKEN + 1-line cortex bump). Next: operator review of the Phase-3 tenancy plan, then Phase-3 build on go.
Deploy note: the cortex-api canary workflow needs the FULL 40-char image sha in image_tag (short sha 404s).
