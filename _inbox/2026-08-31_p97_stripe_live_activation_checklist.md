...
**PHASE 1 COMPLETE as of 2026-09-03**, executed via the Stripe CLI (device-paired, `stripe login`, never a raw key in the planner's session) once the operator confirmed the account was already activated (screenshot showed `pk_live_` key + full "Verify your account" checklist, contradicting the planner's initial generic-activation assumption — corrected in place rather than repeated). Item 4: confirmed live via a real `balance retrieve --live` call, `livemode: true`. Item 5: all 8 prices confirmed already genuinely live (`livemode: true` on every product/price, no test-to-live copy needed). Item 6: `STRIPE_PRO_PRICE_ID`/`STRIPE_MAX_PRICE_ID` set to the sentinel values already established by the test suite (`price_RETIRED_pro_29_must_not_be_used` / `price_RETIRED_max_65_must_not_be_used`) after tracing all three consumers (the real PE ladder checkout, unaffected; the install-scoped PE fallback, operator will remove; the original brokerage/extension checkout, confirmed not actively sold — "Claude extension is just a feature, not gated") — none is an active product needing a real price. Item 7: found an existing live webhook with the exact correct URL/events already present but `disabled`; deleted and recreated it (API never exposes an existing endpoint's secret) to capture a fresh signing secret, pushed to Secret Manager. Item 8: found and disabled the stale test-mode webhook enabled on the same production URL. Item 9: zero promotion codes exist in either mode — nothing to recreate; the live QA promo code (checklist item 9 as it applies to the R2 demo-access window) stays open. Item 10: live Customer Portal configuration created, replicated field-for-field from the test-mode "Default" config (customer_update, invoice_history, payment_method_update, subscription_cancel at-period-end with reason collection; subscription_pause and subscription_update both off, matching test). Item 11: verified directly — every delayed-settlement method (`us_bank_account`, `acss_debit`, `sepa_debit`, `bacs_debit`, `customer_balance`) reads `available: false` on the account's payment method configuration; none reachable at checkout regardless of preference toggles.

Also resolved along the way, discovered while tracing PRO/MAX: `Managed Payments` (the dashboard's "Needs info" nudge on every product) is a wholly separate, opt-in Stripe merchant-of-record product — verified against Stripe's own docs — not a prerequisite for this integration's standard Checkout Sessions. Ignorable.

**Still open in Phase 1/2:** `STRIPE_SECRET_KEY` and `STRIPE_PUBLISHABLE_KEY` — deliberately left for the operator to add directly via their own terminal (`gcloud secrets versions add ... --data-file=-`), never pasted into the planner's session, since those are the two genuinely sensitive values in this list.
