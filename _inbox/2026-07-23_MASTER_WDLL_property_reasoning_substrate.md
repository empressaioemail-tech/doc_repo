...
- 2.5.4 Licensing metadata for revenue routing. | check: if paid atoms route revenue, the atom-contract licensing/source-actor fields the SDK's `RevenueRouter` needs (its `SourceActorReference` is a placeholder today) are landed, OR revenue-routing is explicitly sequenced as pending and the split degrades honestly, not silently. | grade: [ ]
