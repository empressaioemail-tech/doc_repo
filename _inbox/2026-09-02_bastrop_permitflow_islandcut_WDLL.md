...
2. **Bastrop UDC edition selection and citation still resolve correctly under the new tenant.** `bastrop_tx-bdc-2026-adopted` / `BASTROP-UDC` already exist in `code-lookup.mjs` (`AVAILABLE_EDITIONS`, two real sections `14-02-003`/`14-02-008`) — this item re-verifies under real conditions, not template-city's fixture parcels.
   ...
   | depends on: 1
   | grade: [MET] — real engagement created as `bastrop_tx/staff` on real parcel `48021:34737` (Bastrop County, TX), edition declared, matrix run twice on one engagement: `proposedSetbackFrontFt: 30` → Pass on `14-02-003` ("meets the 25 ft minimum"), `15` → Fail ("below the 25 ft minimum"), both with real citations (`editionId=bastrop_tx-bdc-2026-adopted`, `bookId=BASTROP-UDC`). **Correction to this check's own literal wording, source-checked before executing:** `14-02-008` has no adjudication logic anywhere in `mcp.mjs`/`adjudication.mjs` — by design ("not a fabricated Pass") it can never reach Pass/Fail regardless of tenant; confirmed instead that it stays honestly `Unchecked` with a real citation under the new tenant, not regressed to `source-unavailable`. Close `_inbox/2026-09-03_g115-matrix-verify_close.json`.
...
3. **Honest UDC coverage measurement, stated not assumed.** Only 2 sections of the Bastrop UDC are real today. This item measures, against a small sample of real or realistic Bastrop submittal types, what fraction of sections a genuine review would actually touch land as typed `Unchecked` versus a real Pass/Fail — so adoption risk is a known number before staff are asked to use this, not discovered after.
   ...
   | depends on: 2
   | grade: [MET] — N=4 representative submittal types run live across 2 real parcels in 2 zoning districts (SF-1, MU). **Correction to this check's own framing, found before reporting a number:** the real chain data references exactly 2 UDC sections and both are already ingested — section coverage is 2/2 (100%), there are no "missing section numbers." The real, measured gap is one level down: section-ADJUDICATION coverage is 1/2 (only `14-02-003` can ever Pass/Fail; `14-02-008`'s permitted-use check has no logic anywhere, by design), and within `14-02-003`, dimension-adjudication coverage is 1/6 (only front setback is compared; side, corner, rear, max height, and min lot size are real fetched values never adjudicated, named explicitly). Submittal-type invariance confirmed live: all 4 samples produced byte-identical coverage regardless of declared type — `projectType` never reaches `matrixFromChain`. New finding: both sampled parcels' setback-rule atoms carry an unresolved dual-GIS-source conflict on the exact dimension(s) adjudicated, already flagged in the data, never actioned. Close `_inbox/2026-09-03_g115-coverage-measure_close.json`.
