...
6. **Decision 0.20 — operator-confirmed 2026-05-19.** Specs:
   ...
   - **New Neon prod instance:** **separate Neon project** from the hauska-engine substrate stack — keeps blast radius bounded and access control simple (legacy-design-tools holds product operational data; hauska-engine holds substrate retrieval data; these are different concerns; future hauska-engine factor-out per ADR-008 doesn't drag product data with it). Region: closest Neon region to `us-central1` (Neon's actual region naming may differ — pick whichever minimizes Cloud Run → Neon latency). **Plan tier: Neon Scale** — 24/7 compute, autoscaling, database branching, point-in-time restore. The branching + PITR are real safety nets for the cutover and ongoing schema migrations.
   - **Production domain:** **`cortex.empressa.io`** stood up pointed at the new Cloud Run `cortex-api` service BEFORE cutover. Old `prompt-agent-accelerator.replit.app` URL accepted as dying at cutover — `*.replit.app` is Replit-owned DNS and cannot be CNAME-swapped (the planner's initial framing was technically impossible; cc-agent-C surfaced the issue 2026-05-19). Operator confirmed `empressa.io` domain ownership 2026-05-19. Cortex subdomain setup is now Lane C.2 scope, not a long-term follow-on. C.2.5 cutover runbook must include a pre-cutover checklist line: subdomain live + TLS valid + DNS resolves to Cloud Run service before traffic shift. See [`_decisions/2026-05-19_sync_4_5_and_cortex_sprint.md`](../_decisions/2026-05-19_sync_4_5_and_cortex_sprint.md) §Sprint amendments Amendment 4 for the full context.
