...
kind: dispatch
status: HELD - P0b merged (#158; citations are canonical in the stored rows, the gate reads them, no import needed). Sole remaining gate: sprint-54 step-1 gate tenant resolution (AuthContext has product+tier only, no tenant field, so codex_findings_fetch cannot be tenant-scoped). MUST precede 56 step 5. Exception: scope item 3 (cortex_briefing_emit provenance fix) is dependency-free and may land standalone now.
