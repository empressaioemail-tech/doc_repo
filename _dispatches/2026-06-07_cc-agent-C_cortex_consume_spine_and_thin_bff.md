...
kind: dispatch
status: QUEUED - FIRE AFTER engine-core lift (step 4) parity is green; needs the tenant-leg gate seam
...
> **QUEUED.** Fire after the engine-core lift (step 4) reaches behavior parity in the spine, and after the tenant-leg gate seam (54 step 2) is available. This cuts cortex-api over to consume the spine through the gate and removes the migrated engine code, leaving cortex-api a thin product BFF with no ungated path to an engine. Verify identifiers against live source before firing.
...
- Citation/confidence/atomId lineage intact end to end.
