...
8. **Signup + sign-in + reset (hosted page), styled to brand.** The hosted `/api/auth/extension-login` page is bare and sign-in-only, so the panel's signup intent (`intent=signup`, the panel shows "Creating account...") lands on a raw default-browser sign-in form — that is the visible bug. Fix: render a STYLED signup form on `intent=signup` (email / password / confirm / create account), plus sign-in and password reset, all branded to the shared tokens (`hauska.css` from `P:\hauska-brief-extension\Hauska site\`, copied into cortex-api static assets) so the hosted page matches the extension panel. No bare browser form; carry the Hauska wordmark + radar mark.
