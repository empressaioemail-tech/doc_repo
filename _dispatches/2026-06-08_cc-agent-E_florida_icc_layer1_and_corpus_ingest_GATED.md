...
5. **Per-call metering on the licensed tier — the first paid Layer-2 surface (2026-06-10 scope add, sprint 58 moat #2).** The ICC licensed-display tier is the first real paid use case (Layer 1 free, Layer 2 paid per [`08_tiered_access_model.md`](../08_tiered_access_model.md)). The metering substrate is built but dormant (`@hauska-sdk/metering`, Circle/USDC rail, gate metering per the topology). On this cutover, wire per-call metering through the gate on the ICC licensed-text/grounded surface so billing/settlement runs end-to-end on a concrete content tier — the licensed full-text read (or the licensed-display `displayMode` swap) is the metered event; the Layer-1 deep-link/structure read stays free. **Rail-quiet (I7): the rail and any rev-share stay invisible in buyer-facing tool output; the buyer sees the licensed text, not the meter.** Coordinate the gate-side metering hook with cc-agent-M (hauska-mcp-server) if the gate needs a new metering call site; recon and report the gap before building. This is additive to the corpus ingest below and does not gate it — if metering wiring is non-trivial, land the ICC corpus first and file metering as the immediate follow-on. Acceptance: a metered licensed-tier call produces a settlement-ledger entry; an anonymous/Layer-1 call does not; rail-quiet holds in the tool output.

6. **NEC / NFPA grounding track (scoping + partnership note, not ingest).** The NEC is NFPA 70, not on ICC Code Connect, so the ICC deal does NOT cover it. The cortex-local bootstrap (cc-agent-C) seeds NEC interim deep-link reference atoms against NFPA free access (`ungrounded-pending-NFPA`); grounding to full NEC text requires an NFPA data license / NFPA LiNK arrangement, parallel to ICC Code Connect. File/refresh the NFPA licensor line in `73_partnerships.md` as the grounding path and confirm whether NFPA LiNK exposes structured/API access or whether a separate NFPA data agreement is needed. Do NOT ingest or host verbatim NEC text on this run; electrical stays on the interim reference footing until the NFPA track lands.
