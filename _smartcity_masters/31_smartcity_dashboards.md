...
**What the product actually ships, as of 2026-09-16.** `ALL_LENS_IDS` in `smartcity-dashboards` is exactly **nine**: city-manager, development-services, finance, citizen, public-works, parks, police, fire-ems, fleet. Code enforcement, planning, streets, facilities, courts and human resources are in the roster above and are **not lenses in the product** — code enforcement is a Development services tab, not a lens. The roster remains the commercial statement and the nine are the built surface; do not read the roster as a list of things a city can open today. **All nine now have a design** (G-145 complete 2026-09-16), each carrying an adversarial read as a file. Designs and their state: `_design/INDEX.md`. The package to open first is `_design/SMARTCITY_PACKAGE.md`.
...
**CORRECTION 2026-09-16, and three of the nine above do not hold as written.** Measured, with its instrument named, during the G-145 lens design pass. **Verkada is not a running integration at Bastrop.** It occurs zero times in `smartcity-dashboards` `src/vendor-live.mjs`, it is not on `BASTROP_TX.grantedAdapters`, and no Verkada secret exists in `smartcity-os-prod`, `hauska-prod-497015` or `legacy-design-tools-prod`. The camera code in `smartcity-os` is built and fails closed correctly; what is missing is vendor onboarding (OPS-17 G-139). **GoTo is not degraded, it is unauthorised:** it returns `goto_not_authorized` because a human has never completed the OAuth consent. **FirstDue returns a specific 403** because the credential lacks the apparatus and assets scope. Live-verified 2026-09-03 and recorded in that file's own header; Samsara, Spireon and Power BI do return real data. Never state the nine as a flat list of running feeds without this split.
...
4. **Fleet boundary. PRODUCT SIDE SETTLED 2026-09-16; the collateral question remains open.** The product rule is hard and is stated in `src/domains/fleet-vehicles.mjs` itself: a vehicle is **not** an asset. "G-24 stays at zero, and fleet telemetry has been the standing example of the thing that looks like it should fill an asset inventory and must not. The record type is fleet-vehicle under a files-writing vendor kind, never a city-owned asset node." The Fleet lens design draws that boundary positively rather than leaving it implied, and its `check.mjs` refuses asset-inventory vocabulary in an assertive position. So fleet leads as a lens and a vehicle never becomes an asset record. What is still unsettled is only whether collateral leads with fleet as a lens or as an asset class, and that choice may not contradict the product rule above.
...
- 2026-09-16, reconciliation against the shipped product during the G-145 lens design pass. Three of the nine Bastrop integrations corrected: Verkada is not running and has no credential anywhere (G-139), GoTo is unauthorised rather than degraded, FirstDue returns a scope 403. The roster is nine lenses in the product and code enforcement is a Development services tab, not a lens. Fleet boundary settled on the product side: a vehicle is never an asset record. All nine lenses now have a design, each with an adversarial read as a file. Entry point for the whole track: `SMARTCITY_PACKAGE.md` in `_design/`.
