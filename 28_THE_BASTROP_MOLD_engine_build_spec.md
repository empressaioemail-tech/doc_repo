...
8. SMOKE — **NOT MECHANICAL YET.** Today: prose only; no end-to-end live-availability gate in engine-core. This is G5, THE "is the data true and available in the app" benchmark that killed the scan-fix loop. MUST be built as a real fail-closed gate (click N known nodes through the LIVE ledger + map, atoms render, fails loudly) before the fan-out.
