...
| **DA-BIM-Symmetry.** IFC ingest produces `bim-model` atom | Today the `bim-model` atom is produced only on Push-to-Revit per `bimModels.ts`; IFC ingest writes `materializable-element` rows and a glTF bundle but no `bim-model`, so the UI BIM viewport has nothing to render after an architect uploads IFC. User-visible symptom: "IFC ingest doesn't work in the UI" (Nick 2026-05-18). Fix: IFC ingest path at `artifacts/api-server/src/lib/ifcIngest.ts:227-399` also produces a `bim-model` atom on completion. Rides the **Bump 1 window** (post-Sync-1 unlock for downstream Cortex-track work) but is **NOT part of the substrate-v1 planner's Bump 1 cross-PR rollout** — the cross-PR rollout pins consumers to `@hauska/atom-contract@^1.0.0` across 5 repos; this fix is a one-repo behavior change in `legacy-design-tools` and ships as its own targeted PR. No contract change — `bim-model` already exists in the 19-atom registry. Confirmed via 2026-05-19 cross-planner sync. | 2026-05-18 plan-review engine recon §6 + Nick clarification | S |
...
| **DA-6.** Task state UI (L1) | Implement `response-task` atom + UI: persistent checklist state, task transitions, link to findings. **Surface location open** — architect-side `/response` write paths were retired in Task #479 (`EngagementDetail.tsx:3809-3812` per 2026-05-18 Cortex UI recon), so the prior scaffolding does not carry forward. L1 is confirmed in scope (Nick 2026-05-18); pick design-tools vs plan-review side at L1 dispatch time. | Customer-zero L1 |
