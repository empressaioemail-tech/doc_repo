...
status: active
last_updated: 2026-05-23 (DA-12 successor sprint dispatched: 40c's named B.1 + B.2 + 5 power-tool deferrals pulled forward into 40e_cortex_rendering_parity_sprint.md per _decisions/2026-05-23_cortex_rendering_parity_sprint_scope.md; cc-agent-R re-activated, dispatch at _dispatches/2026-05-23_cc-agent-R_rendering_parity_build.md; behind RENDERS_PROD_ENABLED (unchanged); UX-polish bundle (slider, drag-drop, negative-prompt, seed, elapsed timer, share button, ConstellationCanvas) + mask-drawing canvas all in-scope; cortex/render_* MCP retrofit scope widens correspondingly, captured in watch line below. Earlier same day 2026-05-23: DA-12 gap-fill shipped: cc-agent-R Phase A.1 audit 2026-05-22 found the mnml.ai render feature already ~85-90% shipped from the V1 / Spec-54 sprint; operator chose path A gap-fill only; Phase A merged as PR #79 and Phase B as PR #80, both 2026-05-22. Two scope items deferred as follow-ons recorded on the watch line below: B.1 full schema-driven per-expert parameter grid; B.2 image-upload-as-render-source for concept imagery. Activation in prod gated on the operator flipping RENDERS_PROD_ENABLED=true + MNML_RENDER_MODE=live in the same Cloud Run revision. Pre-existing same day 2026-05-22: DA-12 render-output-quality execution home set to 40c_cortex_rendering_sprint.md, Lane 1 of 40b_advanced_capture_features.md activated; cortex/render_* MCP retrofit recorded as a tracked follow-on. Earlier same day: Lane C.4 correction: C.4 endpoints + UI shipped 2026-05-20 via legacy-design-tools PRs #46 + #51, not "the remaining build" as the prior frontmatter and Phase 2 gates said; surfaced 2026-05-22 when cc-agent-C verified against the live repo. #46/#51 predate the _inbox/ courier protocol and never reached this doc; the DA-4/5/6 stream rows, the Phase 2 gates, and the Lane C status block were reconciled 2026-05-22 against the verified legacy-design-tools PR history. Prior: DA-BIM-Symmetry row clarified: the 2026-05-19 Track B IFC schema apply landed on the Replit-side Neon, not the post-cutover cortex-prod, which received Track B + drizzle 0009-0014 only in the QA-04 session 2026-05-21; IFC upload now gated on QA-16 per 43_cortex_qa_backlog.md. Earlier: Cortex QA WS-C added as Phase 2 stream DA-IN-APP-AGENT, in-app chat tool-use, dispatched per 43_cortex_qa_backlog.md. Earlier: Lane A.2 + Lane B Group 3 closed; all 7 Cortex atom types locked in engine atom-registry at @hauska-engine/atoms@0.6.0 — response-task, sheet-content-extraction, attached-document, deliverable-letter, detail-callout-spec, product-spec-reference, deliverable-letter-render; all 24 L-surface MCP tools shipped on hauska-mcp-server main; Sprint Amendment 6 reframed L6 as its own atom — render output IS an atom, not bytes-only; DA-BIM-Symmetry shipped via PRs #28/#29; design-decisions subsection added below MCP co-design block. Lane C.4 endpoints + UI is the remaining build, gated on Lane C.3 close; Group 4 cross-client verification follows. Earlier: combined Cortex/Codex sprint launched per _decisions/2026-05-19_sync_4_5_and_cortex_sprint.md; rendering / image-to-BIM / image-to-CAD descoped to 40b_advanced_capture_features.md)
applies_to: design-accelerator
related: [10_ground_truth, 11_roadmap, 27_engine_evolution_plan, 40_design_accelerator, 40a_customer_zero_observations_arena_roja_2026_05_06, 40b_advanced_capture_features, 41_revit_connector, 48_codex_program_plan, _decisions/2026-05-19_sync_4_5_and_cortex_sprint, adr_001_atom_architecture, adr_007_cross_stakeholder_atom_access, adr_008_engine_factor_out]
owner: nick
---
...
**Rendering / advanced capture (note 2026-05-19, updated 2026-05-23).** mnml.ai integration depth, image-to-BIM, and image-to-CAD were descoped to [`40b_advanced_capture_features.md`](40b_advanced_capture_features.md) with an explicit activation gate. **Lane 1, the mnml.ai render engine, was activated 2026-05-22** as the scoped Cortex rendering sprint at [`40c_cortex_rendering_sprint.md`](40c_cortex_rendering_sprint.md); it is the execution home of DA-12 below. **Status 2026-05-23**: the cc-agent-R Phase A.1 audit found the V1 / Spec-54 sprint had already shipped ~85-90% of 40c; operator chose path A gap-fill only; PRs #79 (backend) and #80 (frontend) merged 2026-05-22; surface stays dark behind `RENDERS_PROD_ENABLED=false`. Image-to-BIM and image-to-CAD (Lanes 2 and 3) stay queued.

**Rendering follow-ons (watch line, updated 2026-05-23).** Three items pulled into the successor sprint [`40e_cortex_rendering_parity_sprint.md`](40e_cortex_rendering_parity_sprint.md) (dispatched 2026-05-23 to cc-agent-R per `_decisions/2026-05-23_cortex_rendering_parity_sprint_scope.md`); one remains tracked as post-activation:

- **B.1 — full schema-driven per-expert parameter grid.** Camera angle × time-of-day × weather × greenery × vehicles × people (10 common params + per-expert grids: exterior 12, interior 8, masterplan 5, landscape 6, product 5, plan 6). The two highest-leverage knobs (expert + style) shipped in 40c. **Pulled into [`40e`](40e_cortex_rendering_parity_sprint.md) 2026-05-23** as Workstream A.3 (schema) + B.1 (UI).
- **B.2 — image-upload-as-render-source for concept imagery.** Needs a new image-upload pipe + `viewpoint_renders` accepting an image source (currently GLB-capture-only). Concept-imagery flow is not end-to-end from a manual sketch upload until B.2 lands. **Pulled into [`40e`](40e_cortex_rendering_parity_sprint.md) 2026-05-23** as Workstream A.4 (schema migration) + A.5 (upload pipe) + B.2 (kickoff UI). Still recommended to land before activation flips, so the concept toggle doesn't ship non-functional to architects.
- **All five power tools** — Render Enhancer, 4K Upscaler, AI Eraser, Inpaint, Style Transfer. Previously typed-but-not-surfaced in the client. **Pulled into [`40e`](40e_cortex_rendering_parity_sprint.md) 2026-05-23** as Workstream A.1 (mnml client methods) + A.2 (api-server routes) + B.3 (5 power-tool dialogs) + B.4 (mask-drawing canvas, required for Eraser + Inpaint).
- **`cortex/render_*` MCP retrofit — scope widens with 40e.** UI-first product MCP retrofit per [`28_mcp_first_product_design.md`](28_mcp_first_product_design.md), lands on hauska-mcp-server after the Renders tab activates. **40e expands the retrofit surface** to cover (a) 5 power-tool MCP tools (`cortex/render_enhancer`, `cortex/render_upscale`, `cortex/render_erase`, `cortex/render_inpaint`, `cortex/render_style_transfer`), (b) per-expert parameter MCP surface (the full B.1 schema reachable from MCP), (c) upload-as-source MCP tool. Retrofit becomes a substantially bigger downstream commitment; operator-acknowledged 2026-05-23 as the dual-interface operational yellow in the 40e premortem.
...
- mnml.ai photorealistic deep integration — per `40b_advanced_capture_features.md` activation gate.
...
| **DA-12.** Render output quality | mnml.ai render engine integrated as the Cortex Renders tab. **Execution home: [`40c_cortex_rendering_sprint.md`](40c_cortex_rendering_sprint.md)** (Lane 1 of doc 40b, activated 2026-05-22). **Status 2026-05-23: gap-fill shipped.** V1 / Spec-54 sprint already shipped ~85-90% of 40c; cc-agent-R path-A gap-fill landed as PR #79 (Phase A, `GET /credits` + Prompt Generator backend) and PR #80 (Phase B, credit badge + deliverable / concept intent toggle + expert / style selects + Prompt Generator affordance). Exit criteria satisfied with two named caveats: B.1 full schema-driven per-expert param grid deferred; B.2 image-upload-as-render-source for concept imagery deferred (the architect-facing concept affordance ships, but the underlying render still captures the GLB — concept imagery is not end-to-end from a manual sketch upload until B.2). Activation in prod gated on operator flipping `RENDERS_PROD_ENABLED=true` + `MNML_RENDER_MODE=live` in the same Cloud Run revision. | DA wave plan W2; `40c` |
