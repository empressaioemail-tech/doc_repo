...
**Deferred rendering follow-ons (watch line, 2026-05-23).** Four items recorded against DA-12 as tracked follow-ons, not in active scope:

- **B.1 — full schema-driven per-expert parameter grid.** Camera angle × time-of-day × weather × greenery × vehicles × people. The two highest-leverage knobs (expert + style) ship; the rest queued without named demand.
- **B.2 — image-upload-as-render-source for concept imagery.** Needs a new image-upload pipe + `viewpoint_renders` accepting an image source (currently GLB-capture-only). Concept-imagery flow is not end-to-end from a manual sketch upload until B.2 lands. Recommended to land before activation flips, so the concept toggle doesn't ship non-functional to architects.
