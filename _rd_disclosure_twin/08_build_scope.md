...
| TW-29 | 4 | **Attention layer (hive).** Aggregate count of participants who marked a band, published as a count of observations rather than as a house call. Weighted by declarer calibration so an ungraded account contributes zero to the published figure | Response states cohort size and composition; below a declared cohort floor the layer says so rather than publishing a thin count; an account with no graded record demonstrably contributes zero, asserted by test; the layer is typed DECLARED INTENT and never as held position | TW-28 |
