...
**`partial` means the populated layer PLUS a determined-absence block covering exactly the missing slice.** Either determined verdict is legitimate there - `absent-verified` when the missing slice was searched and is genuinely empty, `lookup-failed` when that slice could not be reached. Clarified 2026-08-18: an earlier draft said "verified-absence", which read as `absent-verified` only and was narrower than the schema, correctly, permits. It is never a badge on the header and never a softened `populated`. A consumer must be able to see both what was found and precisely what was not, with the absence carrying its own authority and searched scope. Rendering `partial` as `populated` is the worst honesty failure available in this contract, because it is the one case where real data is present to lend credibility to a gap. Specified 2026-08-17 after the design pass found the status declared but never defined.
...
**An honest limit, recorded rather than papered over: the schema cannot distinguish `absent-verified` from `lookup-failed`.** They share a field set, so swapping them still validates. A discriminating required field was considered and REJECTED for two reasons: it would break the uniform absence block this contract deliberately keeps, and `apps/api` already emits a legitimate `lookup-failed` from an unbuilt adapter while both upstreams report `ok`, so any corroboration rule would false-positive on real behaviour. **The distinction is therefore PRODUCER-enforced, not schema-enforced** - whoever writes the absence must choose the right verdict, and that is a review and test property rather than a type property. Recording the limit is more honest than a guard that looks like enforcement and is not.
