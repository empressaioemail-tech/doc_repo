...
3. **B1 — cold-warm RUNS over the six manifests.** Data-only on the reasoning layer, no schema, with the receiving-agent section-number verification gate. **First pass ran 2026-06-10 and is BLOCKED at break-point ([`_inbox/2026-06-09_..._codewarm_runs.md`](_inbox/2026-06-09_legacy-design-tools_cc-agent-C_codewarm_runs.md)):** the verification gate completed (121/155 sample resolved, 78%; Texas-first on `austin_tx`; Texas corpus = 4,754 atoms) but **zero atoms persisted** — blocked by (a) migration 0036 not applied to the deployment Neon (P0, operator action above) and (b) real harness gaps (the parser yields 0 entries for 4/6 manifests with quoted section keys; the URL drivers are hardcoded Florida/FBC-2023). Unblock path: **B1-fix** (harness parser + national drivers + IECC/A117.1 slugs + the R310.4 title correction; [`_dispatches/2026-06-10_cc-agent-C_codewarm_harness_fix.md`](_dispatches/2026-06-10_cc-agent-C_codewarm_harness_fix.md)) lands AND 0036 is applied → **re-fire B1 warm-only** → B2. Fixing the harness (not ephemeral overlays) is what makes it the reusable national template. Dispatch: [`_dispatches/2026-06-09_cc-agent-C_codewarm_runs.md`](_dispatches/2026-06-09_cc-agent-C_codewarm_runs.md) (re-fire after the fix + 0036).
