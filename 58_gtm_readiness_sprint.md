...
| [`austin_2024_uplift_rewarm`](_dispatches/2026-06-10_cc-agent-C_austin_2024_uplift_rewarm.md) | legacy-design-tools | cc-agent-C | B-rewarm | **DONE — PR #164** (correct 2024 package incl. UMC/UPC + TAS; web-verified **0→~35%**, $2.55; IECC fixed 0→32%, IFC 51%). 7 manifests in `_catalog/codes/` |
| [`austin_verified_rate_deepeners`](_dispatches/2026-06-10_cc-agent-C_austin_verified_rate_deepeners.md) | legacy-design-tools | cc-agent-C | deepener | QUEUED — NOT launch-blocking (raise UMC/UPC ~20% + TAS; web-first; IFC remainder = ICC creds) |
...
| [`cortex_intake_chat_wedge_fixes`](_dispatches/2026-06-10_cc-agent-C_cortex_intake_chat_wedge_fixes.md) | legacy-design-tools | cc-agent-C | wedge polish | **MERGED — PR #165** (pre-Revit chat + multi-attach + Opus-4.8 vision; the web-first wedge works; operator e2e on the San Marcos set pending) |
| [`cortex_artifact_ux`](_dispatches/2026-06-10_cc-agent-C_cortex_artifact_ux.md) | legacy-design-tools | cc-agent-C | wedge polish | **DONE — PR #166 held** (auto-nav `agent_artifact` event; letter document view default + Edit toggle + scroll; Download PDF + Print; disclaimer/provenance preserved) |
...
| [`texas_statewide_jurisdiction_gap`](_dispatches/2026-06-10_cc-agent-C2_texas_statewide_jurisdiction_gap.md) | legacy-design-tools (xrepo) | cc-agent-C2 | post-launch (warm-up) | **DONE** — demand-tiered (NOT flat-batch); Tier 1 ~22 ≈$33+22hrs; San Marcos config-ready priority-1. Decision [`_decisions/2026-06-10_texas_coverage_demand_driven.md`](_decisions/2026-06-10_texas_coverage_demand_driven.md) |
...
| ~~`cortex_consume_spine_and_thin_bff`~~ | legacy-design-tools | cc-agent-C | C1–C3 | **SUPERSEDED** by the granular C1/C2/C3 dispatches below |
| [`engine_api_deploy`](_dispatches/2026-06-10_cc-agent-E_engine_api_deploy.md) | hauska-engine | cc-agent-E | C1 prereq | **DONE — LIVE** `hauska-engine-api-00002-mq5` (`https://hauska-engine-api-h7gvu7rgcq-uc.a.run.app`); health ok, /v1/* respond, pysheds works. Follow-up: land the Dockerfile tsx fix on main |
| [`gate_wire_engine_api`](_dispatches/2026-06-10_cc-agent-M_gate_wire_engine_api.md) | hauska-mcp-server | cc-agent-M | fast-follow | QUEUED — for EXTERNAL agents to reach reasoning via the gate (NOT a C1 blocker — cortex-api hits engine-api directly via the seam); + migration 004 |
| [`auth_preserve_anonymous_path`](_dispatches/2026-06-10_cc-agent-C_auth_preserve_anonymous_path.md) | legacy-design-tools | cc-agent-C | auth fix | **DONE — PR #168, CI GREEN** (anonymous→migration-owner, isolation+no-pool tests PASS in CI, minimal sign-in). MERGE → redeploy resolves the lockout + lands #163–168 |
| [`C1_cortex_cut_to_gate`](_dispatches/2026-06-10_cc-agent-C_C1_cortex_cut_to_gate.md) | legacy-design-tools | cc-agent-C | C1 | **IN PROGRESS** (branch `cortex/cut-to-gate`): spine client + flags (off) + findings cut + S1 production wire landed; **pending** briefing/hydro/topo cuts + provenance wiring + push PR. engine-api now live unblocks the calibration-port verify |
| [`C2_extension`](_dispatches/2026-06-10_cc-agent-C_C2_extension_cut_and_unified_signin.md) (status) | legacy-design-tools + extension | cc-agent-C | C2 | **PR-READY (unpushed)** — extension cut + launchWebAuthFlow sign-in built on branch |
| [`C3_thin`](_dispatches/2026-06-10_cc-agent-C_C3_thin_cortex_api_to_bff.md) (status) | legacy-design-tools | cc-agent-C | C3 | **BLOCKED (correctly)** — no code removed; waits for C1+C2 merged + flags verified |
| [`C2_extension_cut_and_unified_signin`](_dispatches/2026-06-10_cc-agent-C_C2_extension_cut_and_unified_signin.md) | legacy-design-tools + hauska-brief-extension | cc-agent-C | C2 | QUEUED → after C1 (extension→gate + execute the unified sign-in task #29 specified) |
| [`C3_thin_cortex_api_to_bff`](_dispatches/2026-06-10_cc-agent-C_C3_thin_cortex_api_to_bff.md) | legacy-design-tools | cc-agent-C | C3 | QUEUED → after C1+C2 verified (remove migrated engine code; lock ungated routes; thin to BFF) |
