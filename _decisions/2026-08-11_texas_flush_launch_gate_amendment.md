...
**DC-1. Denominator coherence.** `summary.totalCells === summary.totalCounties × summary.totalRails` on `GET /api/county-ledger`. Cross-check: `SELECT COUNT(*) AS cell_rows FROM county_rail` equals `summary.totalCells`. Cross-check: `COUNTY_RAIL_DECLARATION.length` (engine source) equals `summary.totalRails`. **Fail** if any pair disagrees.
