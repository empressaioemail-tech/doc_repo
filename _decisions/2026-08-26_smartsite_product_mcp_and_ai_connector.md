...
This decision is not rewritten. v1 of the Smart Site MCP App served on smartsite-mcp p557 (`00063-rej`) with cortex-api p542. v2 is serving on smartsite-mcp p558 (`smartsite-mcp-00065-siv`, URI `ui://smartsite/app-p558.html`) and cortex-api p543 (`cortex-api-00668-cos`), both 100 percent by field name, PRs #553 (`d8dfb319`) and #555 (`24553cfc`). Catalog is 13 tools. P-92 now holds N1 (neighbor seam), F9 (zoning codeRefs), and R3 (`ask_the_map`, still `not_ready`). The persistence tools A-045 listed for P-92 shipped in v1 per A-046. Card `_inbox/2026-08-30_smartsite_mcp_app_v2_WDLL.md`.
