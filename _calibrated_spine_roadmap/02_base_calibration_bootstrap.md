...
There is no fallback onto cities providing data as a partner. The backtest anchor stands or falls on how much historical public-record data we can acquire through public means: public permit and inspection portals, bulk public-records access, and scraping where permitted. Because there is no safety net, acquisition must be maximal in coverage and quality, prioritized to the jurisdictions where the corpus is deep and where we have the most history (Bastrop first as the richest source). Treat acquisition breadth and cleanliness as the gating quality bar for the whole calibration program, not as a preliminary step.
