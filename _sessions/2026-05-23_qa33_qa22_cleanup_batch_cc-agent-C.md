...
status: draft — HR-11 report. Drafted into legacy-design-tools/_research/
  and dropped here for the planner to relocate. Durable copy lives at
  legacy-design-tools/_research/2026-05-23_qa33_qa22_session_close_cc-agent-C.md.
...
The viewer's `classifyElements` (in
`lib/portal-ui/src/components/BimModelViewport.tsx`) walks the
bim-model's elements and skips any with no
...
- 1 synthetic `as-built-ifc-bundle` row: `glbObjectPath = gltfObjectPath`
  iff `parseResult.glbBytes.length > 0`.
...
- (c) bundle row has `glb_object_path` set and the viewer-side
  fetch is the problem (audience gate, ETag mismatch, route 404
...
`gltfObjectPath` never reach logs.

### Fix shape (PR #87)

Two log lines in `artifacts/api-server/src/lib/ifcIngest.ts`. No
behavior change, no schema change, no test impact:
...
The current failure message reads:
...
- schema drift — upstream returns an envelope like
...
(SSL cert-verify failure on refresh), so the canonical "tail the
logs" path is currently unreachable. This PR closes that gap.

### Fix shape (PR #88)

Diagnostic-add only:

- `lib/adapters/src/retry.ts` — when the final attempt returns a
  non-OK response (transient-status exhaustion OR a hard 4xx
  single-attempt), `fetchWithRetry` reads up to 256 chars of the
  body and attaches it as `bodyExcerpt`. The previous "drain and
  discard" branch on retry exhaustion is replaced with a real read
  — the socket is no longer needed since the request is over.
  Whitespace is collapsed so a pretty-printed HTML error page stays
  compact; body-empty / read-threw / transport-reset-mid-read all
  collapse to `undefined` so callers fall back to the bare wording
  they had before.
- `arcgis.ts`, `federal/epa-ejscreen.ts`, `federal/fcc-broadband.ts`:
  ...
  rows).
- `__tests__/retry.test.ts`: six new cases pinning transient-status
  capture, hard-4xx capture, whitespace collapse, truncation with
  trailing ellipsis, empty-body absence, success-path absence,
  mid-read throw absence.
