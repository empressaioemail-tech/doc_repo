---
date: 2026-08-28
topic: CTX six counties served on the full facet shape in production (OPS-19 A-020 to A-025); card F served and re-baked on all six; P-85 next-card opinion
...
| Neon cleanup (A-022) | six stale staging branches deleted 19:37Z after an endpoint-host check against the three hauska-prod store secrets; remaining production, br-super-cloud, br-billowing-queen. **Planner error found at 21:10Z:** the check never enumerated the legacy-design-tools-prod staging secrets, which still bound the deleted br-little-term; the staging-tag redeploy re-resolved `:latest` into it and every staging facets request 500ed (`password authentication failed`, the Neon proxy's answer for a deleted endpoint). Fixed 21:20Z: versions 5 of `STAGING_ATOMS_DATABASE_URL` and `STAGING_DEPLOYMENT_DATABASE_URL` copied from the Factory staging secrets (so the staging serve and the staging bake read the same branches for the first time), staging tag 00646-luj; the two affected bakes were re-walked, not re-baked; memory written |
| Card F re-bakes | Caldwell LIVE on the card F bake (run f4eb42ac 21:37Z, gold LOCKHART 78644 RMD 0.2064 ac); Bastrop LIVE (run 4fc360c7 21:39Z, gold BASTROP 78602 SF-1 0.3827 ac; the screenshot parcel 48021:34729 serves BASTROP 78602 SF-1 0.263 ac); McLennan LIVE (run 66bbd7bd 21:48Z, gold WACO 76711 serves `stamp-missing`, the new verdict on a real unstamped parcel inside a city); Hays LIVE (run 564fa712 22:21Z, gold KYLE 78640, county join gate-blocked so containment and acreage `unmeasured`); Williamson LIVE (run c3c5a9a9 2026-08-29 00:52Z, gold TAYLOR 76574); Travis LIVE (run 5a985b47 01:38Z, gold AUSTIN 78756 `unmeasured` on a no-row join; 6102 Laird Dr `stamp-missing` for Austin with zip and acreage; 48453:227161 `not-applicable` by containment). **Six of six on the card F bake in production; program idle at close** |
...
1. Card F follow-ups (none started): a query point for the 534,700 unstamped rows on the 0,0 bake sentinel so containment can be stated for them; a Factory-side indexed parcel point so BP-VALUE-01 drops its shared-input clause; hauska-map `LAYER_ABSENCE_VERDICTS` gains `stamp-missing` and `unmeasured` so the brief labels the served verdict; parcelJoin state carried into the verdict basis for gate-blocked counties; one atom per parcel per tax year; contract parse before the recorded-instrument insert and `records-request-v1` registered under 49b R1 (from the P-85 opinion).
