...
status: draft — HR-11 report. Durable copy lives at
  legacy-design-tools/_research/2026-05-23_qa33_qa35_session_close_cc-agent-C.md.
...
showed `glbBytesLen=5,234,980` and a populated `gltfObjectPath`.
The backend is fine. The 5MB GLB is being produced, the bundle
row carries `glbObjectPath`, the FE makes 3 .glb requests at HTTP
200 from disk cache, and Three.js receives the bytes. The viewport
still renders nothing — and "the 3d viewer container overruns and
scrolls down on the page indefinitely."

### Root cause

The canvas wrapper at `BimModelViewport.tsx` declares
`aspectRatio: 16/9` plus `minHeight: 280` but no max-height. The
ancestor chain doesn't cap either:
...
`ifcIngest.ts` step 5b's supersession UPDATE was scoped
...
- Per-package typecheck via the Win32 native-deps workaround — all
  7 artifacts + scripts green. Workspace YAML + lockfile reverted
  post-verify.
