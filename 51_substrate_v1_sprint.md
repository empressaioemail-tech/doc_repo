...
- [ ] **First TX batch ingest** (Tier 1 + Tier 2 from 50 starter list):
  ...
  - [ ] Cedar Park
  - [ ] Leander
  - [x] Hutto (UDC loaded as a prioritized one-off 2026-05-20 per [`_decisions/2026-05-20_hutto_tx_prioritized_ingest.md`](_decisions/2026-05-20_hutto_tx_prioritized_ingest.md): 1716 atoms, Path PDF, eval 1.0 / 1.0 / 1.0, platform-internal; eCode360 general code routed to the General Code partnership track)
  - [ ] Elgin
  - [ ] Smithville
  - [ ] Manor
  - [ ] Taylor
...
| A | Atom contract v1.1.0 published with accessPolicy partition | Lane Foundation → A + B | **DONE 2026-05-19** — `@hauska/atom-contract@1.1.0` on npm; v1.1.0 tag pushed; main at hauska-atom-contract commit `6b2c497`. **Shape: ADR-017 `accessPolicy` four-value union reused** (Path R from the dispatch) — wired onto AtomRegistration (type-level default), ContextSummary (per-instance override for the partnership-pending case), and AtomPromptDescription (normalized to `public-free` for downstream filters). Backward-compatible; `^1.0.0` consumers pick up automatically. | Lane A.1 can tag Smithville / Elgin / Bastrop County jurisdiction-corpus instances with `accessPolicy: 'platform-internal'`; Bastrop UDC stays `public-free`. Lane B `list_jurisdictions` filters on `accessPolicy === 'platform-internal'` at the MCP boundary for unauthenticated callers. |
| B | Atom-shape lock per L-surface (Lane A.2) | Lane A.2 → Lane B Group 3 (MCP tools per surface) + Lane C.4 (endpoints + UI per surface) | **DONE — 6 of 6 fired 2026-05-19. Lane A.2 closed.** B(L6) fired with PR #14 `7ed915c` — `deliverable-letter-render` atom (planner Amendment 6 call: render output IS an atom): DID-validated sourceLetterRef + sourceLetterVersion pinning rendered-against-version provenance + opaque blobRef + format enum (DOCX | PDF) + renderedAt + renderedBy actor + tenant-private; 16-test conformance suite including reference-resolution + multi-render 1-to-many tests; atoms 0.5.0 → 0.6.0; 194 workspace tests green. Seven Cortex atom types total in engine atom-registry. cc-agent-E Phase G close-out session summary at doc_repo commit `979118c` captures cross-cutting design decisions (state-as-field + declared eventTypes; discriminant-keyed unions; advisory helpers; leaf composition; tenant-private). **All Sync B fires complete — Lane B Group 3 and Lane C.4 unblocked for every L-surface.** Prior fires: B(L5) PR #13 `b030570` product-spec-reference; B(L4) PR #12 `918f4eb` detail-callout-spec; B(L3) PR #11 `99d4f5f` deliverable-letter; B(L2) PR #10 `30b8047` sheet-content-extraction + attached-document; B(L1) PR #9 `31a8f1f` response-task. Engine PR #7 (accessPolicies query param) merged at `50dd726`. B(L5) fired with PR #13 `b030570` — `product-spec-reference` atom: ProductSpecStatus (active / withdrawn / expired) + ProductIdentifier `{ name, manufacturer }` structured + esrNumber Zod-validated `ESR-<digits>` + inline statusHistory append-only ESR-status-change chain (consumer sees full transition history from one atom version) + ADR-015 actor linking + tenant-private; 17-test conformance suite; atoms 0.4.0 → 0.5.0; 178 workspace tests green. Live-verification poller deferred to runtime layer (legacy-design-tools) per Sprint Amendment 6, out of atom-shape scope. **Only L6 (deliverable-letter render) atom shape pending.** Prior fires: B(L1) fired with hauska-engine PR #9 (`31a8f1f`) — `response-task` atom; atoms 0.0.0 → 0.1.0. B(L2) fired with PR #10 (`30b8047`) — `sheet-content-extraction` + `attached-document` coupled atoms; atoms 0.1.0 → 0.2.0. B(L3) fired with PR #11 (`99d4f5f`) — `deliverable-letter` atom with LetterSection array + per-section provenance to L1/L2/finding/adjudication-state + `deliverableLetterCompleteness()` helper gating L6 send; atoms 0.2.0 → 0.3.0. **B(L4) fired with PR #12 (`918f4eb`)** — `detail-callout-spec` atom: DetailCalloutType discriminated union (door-schedule / wall-section / wall-type / room-finish, v1 extensible) + push lifecycle (pending / pushed / applied / rejected-by-user) with `isLegalPushTransition` advisory helper (applied terminal; rejected → pending re-push allowed) + `apsTaskRef` opaque APS Design Automation work-item ref + source provenance + ADR-015 actor linking + tenant-private; 20-test conformance suite; atoms 0.3.0 → 0.4.0. Engine PR #7 (accessPolicies query param) merged at `50dd726`; parseAccessPolicies docstring fix rode L3 PR. 161 workspace tests green. **L5 (product-spec-reference) and L6 (deliverable-letter render) atom shapes still pending** — cc-agent-E paused awaiting next dispatch. | Per L-surface: Sync B(LN) unblocks cc-agent-M Group 3 LN MCP tool AND cc-agent-C Lane C.4 LN endpoints-plus-UI (the C.4 side also gates on Lane C.3 EngagementDetail split closing). |
| 4.5 | Quality-gated 4-jurisdiction Bastrop-network corpus | 1A + 1D | **DONE (partial) 2026-05-19** — 3 of 4 jurisdictions passed eval at 1.0 / 1.0 / 1.0. Bastrop UDC (181 sections, public-free, born-digital PDF via new RawPdfAdapter — not Municode as the dispatch assumed; PR #5 commit `0ad25f6`). Bastrop County (17 sections, platform-internal, PDF on bastropcounty.gov — not Municode as the dispatch assumed; PR #6 commit `3c256b5`). Elgin (210 sections, platform-internal, Municode JSON mode covering Ch 36 + Ch 46; PR #6). **Smithville deferred** — eCode360 blocks programmatic access (403 + robots.txt disallow); substantive access requires General Code partnership API (parallel bizops, not cc-agent scope). See cc-agent-E session summaries at [`_sessions/2026-05-19_sync_4_5_close_cc-agent-E.md`](_sessions/2026-05-19_sync_4_5_close_cc-agent-E.md) and [`_sessions/2026-05-19_smithville_ecode360_blocker_cc-agent-E.md`](_sessions/2026-05-19_smithville_ecode360_blocker_cc-agent-E.md). Corpus total: 408 atoms (3 jurisdictions) + Grand County 290 = **698 atoms**. | Closes substrate v1 substrate-side work for the next QA cycle. Cost-per-jurisdiction hard-kill checkpoint CLEAR at 4 onboarding events: ~$0 compute + ~3.5 hr human review total, well under target. Smithville will retro-fire when General Code partnership opens the eCode360 API (or Smithville provides alternate source via existing `RawPdfAdapter`). |
| 5 | Quality-gated 20-jurisdiction corpus | 1D → 2D launch gate | **Deferred to public-launch-sequenced demand** per [`_decisions/2026-05-19_sync_4_5_and_cortex_sprint.md`](_decisions/2026-05-19_sync_4_5_and_cortex_sprint.md). 15+ remaining TX cities (Tier 1 Bastrop-network: Round Rock, Pflugerville, Cedar Park, Leander, Taylor, Georgetown; Tier 2 metros: Austin, San Antonio, Fort Worth, El Paso, Plano, Arlington, Irving, Garland, Lubbock, Laredo; Tier 3: Jarrell, Frisco, McKinney, Killeen, plus M9-TBD) sequenced for batch ingest when public-launch timing demands it. Sync 4.5 plus Grand County = 5 jurisdictions cover the operator's actual test surface; the 20-jurisdiction target was the public-launch unblocker, not the internal-QA unblocker. **Hutto (Tier 1) pulled forward 2026-05-20** as a prioritized one-off per [`_decisions/2026-05-20_hutto_tx_prioritized_ingest.md`](_decisions/2026-05-20_hutto_tx_prioritized_ingest.md): the Hutto UDC ingested at 1716 `code-section` atoms, eval 1.0 / 1.0 / 1.0, platform-internal (hauska-engine PR #15); its eCode360 general Code of Ordinances routes to the General Code partnership track. Running catalog total 2414 atoms (698 from Sync 4.5 plus Grand County, plus Hutto 1716). | Public launch unblocked. |
