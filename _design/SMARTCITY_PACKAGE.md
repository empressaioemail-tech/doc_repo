...
## Where things stand: the roadmap and the tracker

**The roadmap** is `_inbox/2026-09-15_roadmap_reconciliation.md`, updated 2026-09-18: five milestones, the order, what blocks each, the critical paths, what the operator owes, and what was promised to Bastrop against what exists.

**The tracker** is `_design/SMARTCITY_TRACKER.md`, GENERATED by `node scripts/govtech/smartcity-tracker.mjs` from the plan of record and every lane close. It fails when a row disagrees with its own close; on its first run it caught G-134 reading OPEN against a closed-partial close. **It is run by hand today, so by the three-question gate it is a report, not yet a control.** Wiring it to a trigger (the doc_repo enforcement workflow, or a pre-push hook) is what would make it one.
