...
> **Architecture-homes reframe (2026-06-21).** The tool counts and gate framing below predate the homes standard. Per the Track C gate rework ([`50_hauska_mcp_server.md`](50_hauska_mcp_server.md), [`_architecture_homes/03_mcp_gate_and_agent_surface.md`](_architecture_homes/03_mcp_gate_and_agent_surface.md)) the gate is now FOUR products (public, codex, reporting, map), 62 tools, with atom_trace, atom_export, and read_atom_calibration added and 1.5.0 read-contract conformance on every read envelope. "Cortex" here means the reporting function package (cortex-api), not a product, per the ADR-008 amendment; the architect product is AEC-cortex. Read facts 2 and the gate split below through that lens; full rewrite owed in the doc scrub.
