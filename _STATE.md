...
- engine-api: `hauska-engine-api-00120-jum` @100% (Sheet Standard v1.1: #169 20-rule rework + #170 §21 vertical rhythm w/ metrics line-boxes + numeric rhythm gate; envelope-canary repointed). Prior: `00114-kiv` (hydrology floor #164 + /nodes #165 + aerial page #166/#167/#168; 4Gi/300s preserved; envelope-canary tag repointed). Project hauska-prod-497015. 4Gi is LIVE-SET only. Deploy = Cloud Build cloudbuild.engine-api.yaml → --no-traffic --tag → smoke → update-traffic. RESIDUAL: cold start can exceed the PE fn 60s cap once (observed on the traffic shift); consider min-instances=1 (operator cost call).
