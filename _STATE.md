...
- engine-api: `hauska-engine-api-00123-muk` @100% (Sheet Standard v1.2: #169 rework + #170 rhythm + #171 frame-clip/named-street-labels/natural-aerial/no-raw-FIPS; envelope-canary repointed). Prior: `00120-jum` (hydrology floor #164 + /nodes #165 + aerial page #166/#167/#168; 4Gi/300s preserved; envelope-canary tag repointed). Project hauska-prod-497015. 4Gi is LIVE-SET only. Deploy = Cloud Build cloudbuild.engine-api.yaml → --no-traffic --tag → smoke → update-traffic. RESIDUAL: cold start can exceed the PE fn 60s cap once (observed on the traffic shift); consider min-instances=1 (operator cost call).
...
- IN FLIGHT: fix/export-consumes-boundary-primitive (engine, W12R) + feat/pe-map-ux-cluster (map/PE, W14: unified toolset, transient chips, true layer toggles incl. zoning-fill unbind, personas removed, brief docking, atom-path base-facts merge).
