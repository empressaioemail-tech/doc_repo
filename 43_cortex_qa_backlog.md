...
| QA-26 | `legacy-design-tools` has `core.autocrlf=true` and no root `.gitattributes`; recurring phantom diffs and empty stashes. | Ops cleanup | WS-F | PR #68 open, awaiting merge — root `.gitattributes` `* text=auto eol=lf` + binary carve-outs. Committed blobs were already LF, so no repo-wide renormalize is needed |
