...
| QA-16 | IFC parse runs inline on the api-server Node main thread behind a non-reentrant `IfcAPI` singleton; a hung or trapped parse wedges the whole cortex-api instance. Isolate the parse in a worker. | Bug + architecture | WS-A follow-on | PR merged 2026-05-21 (one-shot worker_threads isolation). QA-04 still gated: the canary deploy + traffic shift are operator-supervised |
...
| QA-18 | No path to upload client PDFs, photos, and notes to an engagement for the in-app agent to reference. | Feature | WS-F | PR #175 — merge conflicts with main + 3 CI failures (`CitationChip.test.tsx` mock incomplete post-QA-23 integration). Conflict-resolution dispatched to cc-agent-C 2026-05-21, front of queue |
| QA-19 | In-app chat does not auto-scroll while the AI streams a response. | Bug (UX) | WS-F | PR merged 2026-05-21 |
...
| QA-23 | In-app agent presents ungrounded code as confident citations for jurisdictions it has no atoms for (Pagosa Springs / Daulton)CO). | Bug (quality gate) | WS-F | PR merged 2026-05-21 |
