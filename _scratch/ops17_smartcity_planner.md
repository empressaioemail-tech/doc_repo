...
- **`g169-deploy-path-windows` compiled and gate-verified 2026-09-19**: `_dispatches/2026-09-19_g169-deploy-path-windows_dispatch.md`, mission `_catalog/dispatch_missions/mission_g169_deploy_path_windows.md`, canon markers v49001500 / v378cd643 / v2a98086b. Gate proof: real dispatch `allow` (exit 0), and `block` (exit 2) on all three planted violations - CANON-PREAMBLE stripped, no-nesting clause moved off line 1, and the `_inbox/` close path stripped. `P:/tmp/verify-g169-gate.mjs`. **Awaits the operator's hand-carry.**
...
## 2026-09-19 (late) — integration of G-149 and G-169, and the first real deploy run

### GROUND-TRUTH (with timestamps)

- **2026-09-19T18:50Z: `plan-review`'s Cloud Run service PINS 100 percent of traffic to a NAMED revision.** `spec.traffic` reads `revisionName=plan-review-00018-tfv, percent=100`, NOT `latestRevision=true`, read from `gcloud run services describe plan-review --project plan-review-505715 --region us-east1 --format=json`. Consequence: `gcloud run deploy` creates a new revision that then goes `Ready=True, reason=Retired, Active=False` and takes NO traffic. **Any deploy of plan-review's API is a no-op while that pin stands, whatever `deploy.mjs` does correctly.** The service also carries 12 tagged revisions (`g60b` through `g150-reasoner`), so the pinning habit is established.
- **2026-09-19T18:42:36Z: revision `plan-review-00019-5nq` (generation 19) was created by the planner's real deploy run and carries `SERVED_COMMIT=f4aa741717ced34f5d9eeda0448c67fd6e8a2ce5` and label `commit-sha=f4aa7417...`, `ContainerReady=True`.** This is the proof that `deploy.mjs`'s spawn path works end to end. It was Retired without traffic, for the pin reason above.
- **2026-09-19T18:47:48Z: the parity gate exited `DISAGREE`.** API serving `plan-review-00018-tfv` reports `4d3da57999c2208a48f32c22fa82a2c55cd7df4e` (plan-review's own PREVIOUS main, PR #19, 2026-09-18); console `plan-review-app` reports `f4aa741717...`. **The console moved and the API could not, so the planner's authorised run left the two sides measured in disagreement where the pin had held them together.** PR #20's content is inert (`.github/workflows/test.yml`, `package.json`, `scripts/deploy.mjs` — none ships in the web bundle).
- **2026-09-19: `vercel whoami` WORKS on this host** (`Vercel CLI 54.20.1`, team `empressaioemail-tech`). G-169's close states it fails on a TLS certificate; it does not, measured. Do not repeat the certificate story.

### LESSON

- **`smartcity-tracker.mjs`'s `classify()` uses `startsWith` on the cell with bold markers stripped, so the STATUS WORD MUST LEAD the status cell.** Appending `**CLOSED-PARTIAL ...**` at the end of the cell reads as `OPEN`, because the cell still leads with its history word. The tracker refuses with `DISAGREE row=open close=closed-partial` and exit 1, which is the control working. **Hit twice in one day.** The fix is to replace the LEADING token and push the old one into a `HISTORY:` clause.
- **A pinned sha256 in a close may be line-ending dependent, and that is a false-failure trap.** G-169's close pins `588F2B82...` for `scripts/deploy.mjs`; that is the CRLF form. The git blob is LF and hashes to `943A621B...`. A verifier hashing the blob concludes the two repos' files diverged when they have not. **When a pinned hash does not reproduce, try both line endings before reporting a divergence.**
- **`OPS-17_govtech_stack_plan_of_record.md` contains REAL em dashes (U+2014).** PowerShell 5.1 `Get-Content` renders them as mojibake `â€”` (U+00E2 U+20AC U+201D). Copying a shell-rendered substring into a `StrReplace` old_string FAILS. Match with the real character, or anchor on ASCII-only text on either side.
- **A-183's close-artifact gate cannot see a lane that never commits**, because its trigger is `.githooks/pre-commit` and it grades the citation in the commit. G-169's artifacts were UNTRACKED in a worktree whose branch head was main's own commit. Carded as G-175.
- **A canvas missing its `Text` import type-errors on EVERY `<Text>`, and the failure mode is silently confusing.** Without the import, `Text` resolves to the DOM `Text` constructor (`new Text(data?: string)`), so TypeScript reports the whole props object "not assignable to type 'string'" — a message that names neither Text nor the import. `smartcity-roadmap.canvas.tsx` had been missing it (34 identical errors) since it was written, and the canvas still rendered, because a canvas diagnostic is returned in the TOOL RESULT and never surfaces in the UI. **Added `Text` to the import; the check went to zero errors.** A static sweep found no other canvas with the same omission. Generalised: when a canvas check reports "not assignable to type 'string'" on a component's props object, suspect a MISSING IMPORT letting a DOM global shadow the SDK name, not a prop-shape problem.
- **Verify a type diagnosis with a probe canvas rather than by reasoning.** Two probes settled it in two writes: a minimal `<Text tone size>` with only `{H1, Stack, Text}` imported passed (proving the environment fine), and the same element with the full import list minus `Text` failed (proving the cause). Guessing had already produced three wrong hypotheses. **Delete the probe afterwards** so it cannot be mistaken for a real canvas.

### OPEN

- **RESOLVED 2026-09-19: the pin was released and the deploy re-run. G-169 CLOSED.** `spec.traffic` on `plan-review` now reads `latestRevision=True, percent=100`; revision `plan-review-00020-kkj` serves; the parity gate passed on its first attempt, exit 0, `AGREE` on `f4aa741717`. **`scripts/deploy.mjs` exits 0 end to end on Windows for the first time.** Restore path if ever needed: `gcloud run services update-traffic plan-review --to-revisions plan-review-00018-tfv=100`.
- **THE P-170 TRAFFIC LEASE REFUSED THE FIRST SHIFT AND NAMED THE PROCEDURE.** That is the control working, not an obstacle. The procedure: write `_catalog/leases/<service>.json` with `{service, lane, revision, takenAt, expiresAt}` where `expiresAt` is in the future, make the shift, read the serving revision BY FIELD, run the probe, then DELETE the lease. The lease's `lane` follows the `lane_claims.json` convention (a slug plus a seat, e.g. `d12-dashboards-do-cutover` / `cente-vsc-d12`).
- **`smart-files` CARRIES THE IDENTICAL PIN AND IS UNFIXED.** `spec.traffic` pins `smart-files-00013-7pm` at 100 percent with `latestRevision` unset, so its API cannot be moved by any deploy. Carded as G-176. **Do NOT generalise the plan-review fix:** releasing to latest there landed on a revision that already carried the intended commit, which is a property of that service's history rather than a method. On another service the same shift could serve an arbitrary older revision, so resolve the target revision first.
- **G-149's module is on origin as `feat/g149-flood-study` (`ef4e5551`)** and is NOT merged. It existed only in a local branch before; pushing it was preservation, not integration. G-174 owns the screen and the route.
- **Carded this session: G-174** (flood-study screen), **G-175** (A-183's gate cannot see a lane that never commits), **G-176** (the smart-files traffic pin).
- **Throwaway deploy clones live at `P:/tmp/deploy-proof/`.** They exist so the deploy never touched the other lanes' work in `P:/plan-review` and `P:/smart-files`, which carry uncommitted changes and were left exactly as found.
