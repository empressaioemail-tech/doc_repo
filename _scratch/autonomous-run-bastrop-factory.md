...
## PHASE C SF-1 — STEP 3 STOP #2 (2026-08-02): warm!=cert gap → R33 ruling
- GROUND-TRUTH (2026-08-02): SF-1 mixed-vintage FIXED (run 1b: 1501 promoted, 831 honest-declines, stale_latest=0; R10/R27 satisfied). LEDGER-WRITE-PATH WORKS: live /api/county-ledger shows 48021 onboarded=true, zoning+envelope recipe 1.0.0 @ 94.72%, onboardedCount=1. The Phase B gap I owned (console reads a ledger warm didn't write) is CLOSED.
- STEP 3 cert FAILED: 2334/2466 pass, 132 fail (67 frontOrientation/facesAnswer, 54 R32 perEdgeInset, 18 setbacks, 15 district). ROOT CAUSE = warm gate WEAKER than cert gate: warm promotes on verifyWarmCandidateMechanically but cert adds R32-remeasure (measurePerEdgeInsetForRings) + facesAnswer (situs↔OSM road-name match). 126 parcels promote-then-fail-cert. STRUCTURAL (warm!=cert), hits every county — NOT Bastrop-specific.
- RULING R33 (operator-ratified, in the accepted recipe): warm promote-gate must be CERT-EQUIVALENT via SINGLE SOURCE OF TRUTH (warm CALLS the same cert measurement fns before promote); promote-gate==cert-gate (deepens R10). COROLLARY: normalize facesAnswer abbreviations (JR/Junior DR/Drive) FIRST — the failing example (48021:35865 MLK JR DR vs Junior Drive) is a name-format false-negative, correctly oriented; fix the measurement to be RIGHT then share it, don't make warm strict against a too-strict cert. Genuine wrong-edge still fails.
- PR #210 (reproducibility patches) CI RED: type errors in honest-decline-promote.ts (emptyReason not a valid prop; wrong envelope-kind literal; number vs WidthedPointEstimate). Fix before merge.
- DISPATCH: PHASE_C_RESUME_R33_warm_cert_alignment.md — fix #210 → implement R33 (normalize facesAnswer + share cert measurement in warm gate) → re-warm SF-1 → re-sweep to blockPass → continue GC/MU/RR/PI/IND → stop at R6.

## MAJOR CORRECTION (2026-08-02) — operate-not-rebuild; the fleet built a new factory instead of running the existing one
- OPERATOR FLAGGED (huge failure, fix at root): task was START+TEST the existing proven factory with Bastrop — NOT build a factory. Fleet re-built the cohort+cert WRAPPERS (new bastrop-district-cert-grade.mjs vs proven block13-cert-grade.mjs; bbox/roster-field cohort) and debugged its own machinery 3 STOP cycles. Every "finding" = wrapper-vs-proven divergence, not data.
- ROOT CAUSE (systemic): recursive-loop L3 built for the PRODUCT (PE onboarding), NOT the fleet. Fleet coherence carrier = hand-carried prose; no divergence gate; "operate the template" was prose that got interpreted-away. 64_recursive_loop/04 ALREADY documented this exact gap a month prior — never built.
