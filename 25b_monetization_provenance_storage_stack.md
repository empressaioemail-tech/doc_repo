...
4. METERING (`@hauska-sdk/metering`, `McpMeteringGate`) = the GATE HOOK. Authorizes a paid read at call time (gate-then-serve), decrements bundles, triggers overage checkout. **ARMED as of 2026-08-22** (correction; this line previously read "NOT wired into the live gate today"): `POST /mcp` runs authMiddleware, then `requireProduct` or `authorizePaidRead`, then `sdk-metering.ts` `gate.authorizeCall({keyId, tier, layer: 2})` into `McpMeteringGate`, writing `sdk_metering_usage` on allow, with `SDK_METERING=1` in `cloudbuild-mcp.yaml`. Authorize runs before serve and Stripe is out of the money path. Outbound payout remains dormant: `RevenueRouter` has zero call sites and no webhook, and the ICC actor fixture carries no `perReferenceRateMinor`, so accrual rows land `amount_minor: null`. Verified by the substrate seat against deployed main `b5f26de`. Model context for this stack now sits above it in [`19_the_instrument_contract.md`](19_the_instrument_contract.md).
