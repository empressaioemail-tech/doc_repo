...
| 2 Fix what is wrong | native-D8 math (the local fallback; pysheds-on-spine already resolved audit #3 via the flip); **drainage latency** (prod soak 2026-06-11: `/v1/hydrology/drainage` took ~100s on the spine, pysheds correct but the D8 grid/catchment-buffer/resolution needs profiling; `/v1/hydrology/rainfall-forcing` ~10.8s also worth a glance; findings-generate ~19s and briefing-generate ~29s are LLM-bound and acceptable, so drainage is the lone outlier); precedence fires in prod and ports into the spine; `snapshotDate` from vintage; topography/hydrology coverage signals; grounding holes (briefing payload, corpus edition/sourceUrl) | audit #3,4,8,9,10 + soak | E + C | Wave 1 contract |
