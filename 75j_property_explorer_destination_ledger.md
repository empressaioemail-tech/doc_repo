...
related: [2026-07-20_map_first_program_launch, 2026-07-20_what_separates_us_service_elevation, 2026-07-21_architecture_gaps_node_facets_atomization_and_gated_functions, 2026-07-21_overpass_road_data_spec, 2026-07-20_provable_county_data_pipeline_design, 09_post_saas_substrate_thesis, 08_tiered_access_model]
owner: nick
---
...
| 3 | Code & permit depth | Authoritative building-code answers (ICC I-Codes ingested) + the permit path — "what the code requires to build this." Kills the "check with the city" hedge. | ICC infra built; credentials/ingest pending; not on the map surface. | 25 |
...
| 5 | Three personas, one engine | Homeowner verdict / investor envelope / architect citation off the SAME facts. "Three registers, one truth." | laySummary + persona lenses exist; not fully flexed per-persona on this surface. | 40 |
| 6 | Reports + visualizations (paywalled) | Generate a cited property report with MAP-level and REPORT-level visualizations of the answer (envelope drawn, constraints mapped, flood/topo rendered) — a shareable deliverable, behind the paywall. | "Research this" stubbed; report gen exists elsewhere, not wired to this surface. | 15 |
...
| 11 | Surface — desktop extension | Optional Chrome add-on layered on the web-app account — listing-site capture, hands the property to the web app. | Extension exists; substrate migration held for smoke; not yet an add-on to the web account. | 40 |
| 12 | Surface — mobile / on-site (PWA) | Installable phone surface for standing-on-the-parcel: full-bleed map, GPS, the answer in hand at the site. | GPS wired in the web app; not a PWA / not on-site-optimized. | 20 |
| 13 | Auth + paywall + tenancy | Web OAuth front door; user-aware entitlement (not install-keyed); tenant-isolated storage; accessPolicy on atomized facets = the paywall mechanism (free-public vs paid vs tenant-private). | Anonymous-only; auth/tenant = sprint-54, held; accessPolicy not enforced on facets. | 10 |
| 14 | CRM integration | Pipeline/CRM hooked up — leads, saved properties, user journeys flow into the CRM (as the trading app does). | Not wired on this surface. | 5 |
| 15 | GTM system (model = trading app) | A captured AND built GTM system; the trading app is the reference (it's a GTM machine): market footprint per vertical, funnel, the motion to own the MCP market. | Strategy captured in docs; the SYSTEM not built here. | 15 |
