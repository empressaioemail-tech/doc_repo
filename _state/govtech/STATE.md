...
**G-116 Phase 2 — vendor credentials TRANSFERRED (A-113); `smartcity-os` platform-read endpoint LIVE (A-114).** Found and copied real credentials for **7 of 9 vendors** from `smartcity-os-prod`'s Secret Manager into `smartcity-dashboards`'s own (25 values — MyGov, Samsara, OpenGov, FirstDue, GoTo, PowerBI, Spireon — byte-verified). Skipped `smartcity-PIPEDRIVE_API_TOKEN` (program rule: never Pipedrive as a city feed) and smartcity-os's own internal app secrets. **Esri and Verkada confirmed to have no credential anywhere** — need real vendor onboarding, not a transfer.

**The absolute-no-touch line on `smartcity-os` was crossed once, narrowly, on explicit operator authorization** (`_decisions/2026-09-03_smartcity_os_platform_read_authorization.md`) — this is a real change to that repo, not read-only anymore for this one case. Built `GET /api/platform/mygov/permits`: server-to-server read of Bastrop's already-synced active permits, gated by a new `PLATFORM_INTERNAL_API_KEY`, reusing the file's existing query shape and its own documented `in_mygov_active_list` accuracy contract. **A real bug caught before any production traffic shift**, live-testing a tagged 0%-traffic revision rather than trusting unit tests: a global session-auth gate (`isAuthenticated` on every `/api/*` path except an allowlist) intercepted the new route before its own auth ever ran — the `buildTestApp()`-based unit tests never exercised that gate. Fixed by adding `/platform` to the same allowlist `/vfd`/`/grant-writer` already use for their own non-session auth. Deployed with full tag→smoke-test→shift discipline given the stakes: wrong-key/no-key refused, correct-key returns real live permit data, existing session route confirmed unaffected — only then shifted traffic. Live: `smartcity-api-00121-xez`@100%.

**Consumer side also built and LIVE (A-115): the first real, non-fixture domain feed on this product.** `src/mygov-permits.mjs` reads the platform endpoint and maps rows onto the record envelope with `origin: "feed"`. Deliberately NOT inside `composeDomain`/`fixture-seam.mjs` — that file's own header says it isn't a feed, and its guards exist to catch fixtures that look real. Branches at the `server.mjs` route level instead (matching `meetingsFromPack`'s pattern for municode); `domains.mjs` and its other 9 consumers untouched, avoided a 10-file async ripple. **Real permit statuses are NOT force-mapped onto the fixture's invented four-stage taxonomy** — real values (`active`/`in-review`/`pending`/`completed`) barely overlap the fixture's (`overdue`/`awaiting-applicant`/`in-review`/`ready-to-issue`), and `smartcity-os`'s own code calls the source field "unreliable for permits." Real records keep their real status; a new `extras.realStatusCounts` groups honestly instead. Live-verified directly against the real endpoint before deploy: 312 real active Bastrop permits, correctly mapped. Deployed: `smartcity-dashboards-00044-red`@100%. `template-city` confirmed unaffected.

**What's still genuinely unbuilt:** granting an adapter still only changes wording for the other 10 domains (A-112's finding, now closed for exactly one — MyGov permits). The HTTP route itself is correctly access-gated (403) behind the still-unresolved Hauska tenant-key gap for `bastrop_tx` (named in A-112/A-114, not new) — full browser-facing verification waits on that. Phase 2 for the remaining 9 vendors (Samsara, OpenGov, FirstDue, GoTo, PowerBI, Spireon, plus Esri/Verkada which need real onboarding) is unstarted; each would need its own "does smartcity-os already have this working" check before assuming a from-scratch build, the way MyGov did.
