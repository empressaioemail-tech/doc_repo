...
**RESOLVED 2026-09-03, then DEPLOYED LIVE same day.** All four remaining rows (A-062, P-100, P-101, P-106) merged across both repos — LDT #583/#580/#586/#584, hauska-map #334/#337/#335 — each verified on the actual CI check-run conclusion string. Migrations `0093` and `0094` applied to production, cortex-api and smartsite-mcp both canary-deployed and traffic-shifted to the new revision, hauska-map/property-explorer redeployed on Vercel (it does not auto-deploy on push, confirmed by zero deploys in the 20h before three merges landed). Full detail, verification evidence, and one real gotcha caught (guessed route paths wrong twice, corrected by reading source instead of trusting memory): OPS-16 A-078 and A-079.
...
~~P-106's unmeasured threshold. The lane proposes fifty percent on a bimodal distribution: every live rail sits between zero and fifty-seven percent unmeasured, every dead one is exactly one hundred, and nothing exists between fifty-eight and ninety-nine. At fifty it refuses eight of thirty-six rail-county pairs and every county keeps at least three usable rails. It ships no default, so unset means no gate.~~ Ruled 2026-09-04: default accepted as-is ("hard to say without testing"). See OPS-16 A-096.

~~P-106's zoning divergence. The projection derives absent-verified from two independent inputs where the serve path derives it from one and says unknown. They disagree on 346,165 parcels and the projection is right. The question is whether to teach the serve path the better rule, which turns those parcels from "we do not know" into a real answer.~~ Ruled 2026-09-04: serve path should read NOT_APPLICABLE for these parcels, matching the projection. Likely already inside the concurrent CTX pipeline wrap-up sprint's `ctx-wrapup-factory` scope (its "not-applicable cell-state fix"); flagged there rather than dispatched a second time from this seat. See OPS-16 A-096.

~~P-110's metering. The Hauska export tools are SDK-metered per call while these callers already paid Stripe for Studio. Hauska Inc and Legacy Group ATX are separate entities, so this is intercompany billing rather than accounting. Nobody can wire the export path until it is answered, because the answer decides whether exports are profitable at one hundred and twenty-nine dollars.~~ Ruled 2026-09-04: attribute to Legacy Group ATX's side, no call into Hauska's per-use metering for these exports. The intercompany settlement question is explicitly deferred as its own separate topic — this ruling unblocks the engineering only. Ready to dispatch. See OPS-16 A-096.

~~P-112's WorkOS posture, per section 3.~~ Ruled 2026-09-04, see section 3.

**Decisions with no lane waiting.** ~~The GoHighLevel sub-account posture, cheapest to change now while no contact exists.~~ Ruled 2026-09-04: two sub-accounts. Existing `Smart Site` sub-account (`KtUXFiFB5e22abpLp1MR`) confirmed correct for subscribers + affiliates; a second for SmartCity OS/municipal deferred until that product migrates off its current CRM. Also corrected the same exchange, not just decided: masters 06's "no sales CRM" line was itself wrong when written, not superseded — Smart Site runs a low-touch human sales process. See `_smartsite_masters/06` and OPS-16 A-096. ~~The four affiliate segment lines.~~ Ruled 2026-09-05: builder/designer line corrected to lead with Feasibility + Flood & Drainage; other three confirmed as-is. See OPS-16 A-103. ~~Whether valuation output may surface.~~ Ruled 2026-09-05: tax-assessed CAD data may display, gated same as owner info; market-opinion valuation stays refused per masters 06. See OPS-16 A-103. ~~The "brief" glossary conflict.~~ Ruled 2026-09-05: MCP connector needs the same brief-level snapshot the web dock shows, as a snapshot not a report. See OPS-16 A-103. ~~Whether the extension still sells Pro and Max.~~ Ruled 2026-09-05: extension is retired, has been for months; Pro/Max must fail closed, not resolve to a stale test id. See OPS-16 A-103. ~~Whether `dossier` means the same thing on both surfaces.~~ Ruled 2026-09-05: the word `dossier` is retired; X-ray is the document; connector must match the web app's package table (P-119), not the reverse. See OPS-16 A-103. ~~Whether the share plane requires a `graph_opt_in`.~~ Ruled 2026-09-05: no, first-party counting, no consent required; check/update Terms of Service to cover it. See OPS-16 A-103.
