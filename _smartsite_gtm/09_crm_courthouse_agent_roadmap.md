...
| R5 | Courthouse records: reliability hardening (block-parser widen, Caldwell/McLennan live) | RUNNING as a background subagent lane-planner (this session), launched 2026-09-03 after operator explicit override of the standing hand-carry-only decision. Task 3 (re-running the 14 held jobs) is hard-gated in its dispatch and will NOT execute without a fresh operator confirmation — see R8 | P-113 | `_inbox/2026-09-03_p113-reliability_cp1.json`, `_cp2.json`, `_close.json` | Await agent completion notification; read the close artifact and the actual PR/diff before treating as done, per this operation's rule that a subagent's success report is a claim, not a fact |
| R6 | Courthouse records: agent exposure via MCP | RUNNING as a background subagent lane-planner (this session), launched 2026-09-03 same override | P-113 | `_inbox/2026-09-03_p113-mcp_cp1.json`, `_cp2.json`, `_close.json` | Await agent completion notification; read the close artifact and the actual PR/diff before treating as done |
...
| R12 | Portal-access ruling reconfirmation (gates P-113 Task 3, re-running the 14 held jobs found by the 2026-08-31 block-job audit) | CONFIRMED 2026-09-03 — operator states the ruling still stands. Relayed directly to the running reliability subagent, which is instructed to record this confirmation and its source in its close artifact before executing Task 3 | P-113 | An explicit operator statement that the 2026-08-26 / Measurement X3 2026-08-30 portal-access ruling still stands as of today | Closed; verify the close artifact actually records the confirmation and source before treating the 14-job re-run as legitimate |
