...
5. **`CLAUDE.md`** — portfolio constitution: entities, brands, the four structural commitments, what is
   settled, what is out of scope. Loaded automatically by Claude Code and by Cursor (it carries
   `alwaysApply` frontmatter).
6. **`_catalog/repo_map.md`** — what every directory in this repo is, who owns it, and whether it is in
