...
4. **Ledger env values, concretely:** `LEDGER_INGEST_URL` = the cortex-api Cloud Run URL (`gcloud run services describe cortex-api --project legacy-design-tools-prod --region us-central1 --format 'value(status.url)'`); `LEDGER_INGEST_KEY` = `gcloud secrets versions access latest --secret SERVICE_API_KEY --project legacy-design-tools-prod` (trim trailing newline). Atoms `DATABASE_URL` = secret `DATABASE_URL` in `hauska-prod-497015`.
