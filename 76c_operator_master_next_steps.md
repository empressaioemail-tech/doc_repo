...
- [x] cc-agent-C parcel/zoning adapter + **OAuth2 rework** — **LANDED 2026-06-06**. **PR #141** (branch `cortex/cotality-adapter-scaffold`, HEAD `ddb7029`), 262/262 tests green, typecheck clean, **HELD for operator merge**. OAuth2 `client_credentials` per demo app (Property + SpatialTile used; RiskMeter reserved). Close notes: [`scaffold`](_inbox/2026-06-06_legacy-design-tools_cc-agent-C_cotality_adapter_scaffold.md), [`oauth rework`](_inbox/2026-06-06_legacy-design-tools_cc-agent-C_cotality_oauth_rework.md). Spec: [`_dispatches/2026-06-06_cc-agent-C_cotality_oauth_rework.md`](_dispatches/2026-06-06_cc-agent-C_cotality_oauth_rework.md).
- [x] Six demo creds created + mounted on cortex-api (`cortex-api-00119-laq`): `COTALITY_{PROPERTY,RISKMETER,SPATIALTILE}_{KEY,SECRET}` (lengths verified 48/64).
- [ ] **Operator smoke (gates merge + everything downstream):** confirm 5 endpoint constants from portal API DOCUMENTATION — `COTALITY_TOKEN_URL` (default `api-prod.corelogic.com/oauth/token`), `COTALITY_PROPERTY_BASE_URL`, `COTALITY_SPATIALTILE_BASE_URL`, `COTALITY_PROPERTY_POINT_PATH`, `COTALITY_SPATIALTILE_POINT_PATH`, plus OAuth body format (form vs Basic); set env overrides on cortex-api if defaults differ; run generate-layers smoke on 1904 Heathwood Cir; then merge PR #141.
