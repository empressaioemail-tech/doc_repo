...
status: active
last_updated: 2026-05-22 (CDX-Phase1-1 reversed: the reviewer-side review surface relocates into the Design Tools app Findings tab per _decisions/2026-05-22_codex_review_surface_relocation.md; the standalone codex-reviewer-qa artifact is retained as the smartcity-os production-surface reference. Prior: CDX-Phase1-1 resolved 2026-05-21 as a standalone codex-reviewer-qa artifact; combined Cortex/Codex sprint launched per _decisions/2026-05-19_sync_4_5_and_cortex_sprint.md; Phase 2 stream CDX-MCP added for existing-product Codex MCP tool exposure.)
applies_to: codex
related: [11_roadmap, 27_engine_evolution_plan, 28_mcp_first_product_design, 42_design_accelerator_program_plan, 45_codex_qa_scenarios, 47_codex_plan_review, 50_hauska_mcp_server, 51_substrate_v1_sprint, 05_living_lineage_thesis, 06_cities_value_narrative, 30_smartcity_os, 40_design_accelerator, _decisions/2026-05-19_sync_4_5_and_cortex_sprint, adr_001_atom_architecture, adr_007_cross_stakeholder_atom_access, adr_008_engine_factor_out]
owner: nick
---
...
| **CDX-Phase1-1.** Reviewer-side QA surface location decision | **Reversed 2026-05-22** per [`_decisions/2026-05-22_codex_review_surface_relocation.md`](_decisions/2026-05-22_codex_review_surface_relocation.md): the reviewer-side review surface relocates into the Design Tools app's Findings tab; the standalone artifact is retained as the smartcity-os production-surface reference. Prior resolution, **2026-05-21** per [`_decisions/2026-05-21_codex_reviewer_qa_surface_location.md`](_decisions/2026-05-21_codex_reviewer_qa_surface_location.md): the reviewer-side QA surface lives as a new dedicated `codex-reviewer-qa` artifact in `legacy-design-tools`. Scaffold delivered by cc-agent-C as PR #66, merged 2026-05-21: a sixth artifact `artifacts/codex-reviewer-qa/`, a Vite/React SPA shell with router + QueryClientProvider wired, built into the Dockerfile and served at `/codex-reviewer-qa`. Phase 2 reviewer surfaces CDX-3/4/5 dispatched to cc-agent-C 2026-05-21 ([`_dispatches/2026-05-21_cc-agent-C_codex_phase2_reviewer_surfaces.md`](_dispatches/2026-05-21_cc-agent-C_codex_phase2_reviewer_surfaces.md)); CDX-9 held behind DA-5. | This plan | S |
...
| **CDX-9.** Comment-letter auto-draft | Generate comment letter as `deliverable-letter` atom + DOCX/PDF render. Reuses DA-side pipeline established in `42_*` DA-5 (shared deliverable-letter implementation). **Delivered 2026-05-22** by cc-agent-C as PR #72 (open, CI green) per [`_dispatches/2026-05-22_cc-agent-C_cdx9_comment_letter.md`](_dispatches/2026-05-22_cc-agent-C_cdx9_comment_letter.md): a "Draft comment letter" action on the codex-reviewer-qa surface, reusing the existing Cortex L3/L6 deliverable-letter pipeline (L3 PR #48 + L6 PR #51, Lane C.4). Planner ruled finding-intrinsic adjudication satisfies the per-section provenance commitment; no Codex adjudication-state atom. | `47_*` Wave 4 |
...
| **CDX-QA-1.** Codex 1b QA scenarios documentation | Write the QA scenario set: which projects, which jurisdictions, expected atom outputs at each step, what "wrong" looks like for each. Durable QA spec; future regression tests run against it. **Delivered 2026-05-22** as [`45_codex_qa_scenarios.md`](45_codex_qa_scenarios.md): four scenarios (S1 primary end-to-end pass on Musgrave, S2 second-project confirmation, S3 Bastrop jurisdiction switch, S4 failure-mode catalog) operationalizing the 8-point QA-readiness definition. (The DA-8 Cortex equivalent it was to mirror is not yet written; 45 establishes the form.) | This plan |
...
- ~~**Reviewer-side QA surface location in legacy-design-tools**~~ — **Resolved 2026-05-21** as a new dedicated `codex-reviewer-qa` artifact; **reversed 2026-05-22** — the review surface relocates into the Design Tools app's Findings tab per [`_decisions/2026-05-22_codex_review_surface_relocation.md`](_decisions/2026-05-22_codex_review_surface_relocation.md), the standalone artifact retained as the smartcity-os production-surface reference.
