...
**Corrected 2026-09-04 — both paragraphs above were wrong, not superseded; operator named the original as a misprint.** Smart Site DOES run a human sales process against its own contacts, low-touch rather than the demo-booking/rep-queue shape the retired text disclaimed. GHL pipeline and sequence machinery may legitimately point at Smart Site subscriber and affiliate/partner contacts as part of that low-touch process — this is not a leak to be guarded against, it is how the product's sales motion actually works. Exact stages/cadence of the low-touch process are not yet specified here; do not infer a shape beyond what is stated. What does NOT change: SmartCity OS (municipal) stays on its own, separate, pre-existing CRM for now — folding it into GHL is a later, distinct migration, out of scope until the operator raises it — and CRM tooling for Empressa Solutions custom builds is unaffected. Full detail: OPS-16 A-094.

**GHL sub-account posture, resolved 2026-09-04** (operator ruling): two sub-accounts, split by motion. The existing sub-account (`KtUXFiFB5e22abpLp1MR`, named Smart Site) is confirmed the correct, already-in-use account for Smart Site subscribers and affiliate/partner recruiting — no change needed there. A second sub-account for the SmartCity OS / municipal motion is deferred until that product actually migrates off its current CRM.
