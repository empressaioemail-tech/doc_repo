...
| smartcity-dashboards | Empressa product: SmartCity Dashboards (doc 31). Own repo, own service. GitHub `empressaioemail-tech/smartcity-dashboards`. Local `P:\smartcity-dashboards`. Neon pooled `ep-still-wave-avbwm4yc-pooler` db `neondb` (DSN gitignored). GCP `smartcity-dashboards` / `666199866241`. | Template then Bastrop cutover. Mounts Hauska spine + SmartSite + Smart Files over G-13. Tenant-registry Neon is this product's store, not a copied parcel/files table. MCP tools on hauska-mcp-server PR 70. | Not `smartcity-os`. Not Asset Management. Not a repo per city. Do not clone live Bastrop wallpaper. Do not deploy into `smartcity-os-prod`, `hauska-prod-497015`, or `legacy-design-tools-prod`. Not Hauska substrate. |
