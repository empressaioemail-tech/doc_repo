...
| smartcity-dashboards | Empressa product: SmartCity Dashboards (doc 31). Own repo, own service. GitHub `empressaioemail-tech/smartcity-dashboards`. Local `P:\smartcity-dashboards`. One product, cities as tenant packs. No Neon and no GCP yet. | Template then Bastrop cutover. Mounts Hauska spine + SmartSite + Smart Files over G-13. MCP tools retarget on the existing Hauska MCP server. | Not `smartcity-os`. Not Asset Management. Not a repo per city. Do not clone live Bastrop wallpaper. Do not deploy into `smartcity-os-prod`, `hauska-prod-497015`, or `legacy-design-tools-prod`. Not Hauska substrate. |
