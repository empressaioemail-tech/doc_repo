...
export interface FloodZoneShare {
   ...
   *
   * AMENDMENT 4.3 (2026-08-18, planner). A share is `1` ONLY when something
   * MEASURED it as 1. A set of length one does not imply a share of one.
   *
   * A scalar upstream reports one zone; it does not report that the parcel lies
   * wholly within it. Deriving `1` from set length asserts total containment
   * that was never measured, and destroys exactly the information the operator
   * corrected us on: a home can sit in the 100-year AND the 500-year
   * floodplain, and a lot can be part AE and part AO. That this happens is not
   * hypothetical, the two flood stores disagree AO against AE on parcel
   * 48021:36521 today.
   *
   * A sibling `method` field is not sufficient protection. The lesson of this
   * whole programme is that a value which looks like an answer gets consumed as
   * one no matter what a neighbouring field says.
   */
 ...
 */
export declare function composeVerdict(sheet: ParcelFactSheet): string;

/**
 * The one place a measurement becomes text. Pure. A renderer that formats a
 * Measurement by hand is in breach.
 */
export declare function formatMeasurement(
  m: Measurement,
