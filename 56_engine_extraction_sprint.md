...
5. **Cut consumers to the gate.** Each app consumes engine reasoning through the gate seam; lock cortex-api's direct engine routes. Depends on step 4 + the tenant-leg gate work ([`54`](54_tenant_leg_sprint.md)). **The gate-citation-lineage closure dependency is SATISFIED** ([`_dispatches/2026-06-09_cc-agent-M_gate_citation_lineage.md`](_dispatches/2026-06-09_cc-agent-M_gate_citation_lineage.md), merged #30 + #159, 2026-06-09): the gate threads finding citations on fetch (P0a) and preserves them on override (P2), so routing adjudication through the gate no longer silences the arrow-two deposit loop. Tenant resolution (54 step 1) is also met (#29). Remaining: the gate-front seam (54 step 2) and engine-core lift (steps 3-4).
